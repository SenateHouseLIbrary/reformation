(function(root){root.VCO={VERSION:"0.1",_originalL:root.VCO}})(this);VCO.debug=!0;VCO.Bind=function(fn,obj){return function(){return fn.apply(obj,arguments)}};trace=function(msg){if(VCO.debug){if(window.console){console.log(msg)}else if(typeof jsTrace!="undefined"){jsTrace.send(msg)}else{}}};VCO.Util={extend:function(dest){var sources=Array.prototype.slice.call(arguments,1);for(var j=0,len=sources.length,src;j<len;j++){src=sources[j]||{};for(var i in src){if(src.hasOwnProperty(i)){dest[i]=src[i]}}}
return dest},setOptions:function(obj,options){obj.options=VCO.Util.extend({},obj.options,options);if(obj.options.uniqueid===""){obj.options.uniqueid=VCO.Util.unique_ID(6)}},findArrayNumberByUniqueID:function(id,array,prop){var _n=0;for(var i=0;i<array.length;i++){if(array[i].data[prop]==id){trace(array[i].data[prop]);_n=i}}
return _n},convertUnixTime:function(str){var pattern=/^(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/;if(str.match(pattern)){var date_parts=str.match(pattern).slice(1)}
var date_array=[];for(var i=0;i<date_parts.length;i++){var val=parseInt(date_parts[i]);if(i==1){val=val-1}
date_array.push(val)}
date=new Date(date_array[0],date_array[1],date_array[2],date_array[3],date_array[4],date_array[5]);months=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];year=date.getFullYear();month=months[date.getMonth()];day=date.getDate();time=month+", "+day+" "+year;return time},setData:function(obj,data){obj.data=VCO.Util.extend({},obj.data,data);if(obj.data.uniqueid===""){obj.data.uniqueid=VCO.Util.unique_ID(6)}},mergeData:function(data_main,data_to_merge){var x;for(x in data_to_merge){if(Object.prototype.hasOwnProperty.call(data_to_merge,x)){data_main[x]=data_to_merge[x]}}
return data_main},updateData:function(data_main,data_to_merge){var x;for(x in data_main){if(Object.prototype.hasOwnProperty.call(data_to_merge,x)){data_main[x]=data_to_merge[x]}}
return data_main},stamp:function(){var lastId=0,key="_vco_id";return function(obj){obj[key]=obj[key]||++lastId;return obj[key]}}(),isArray:function(){if(Array.isArray){return Array.isArray}
...
return str};if(prefix){return prefix+"-"+randomID(size)}else{return "vco-"+randomID(size)}},htmlify:function(str){if(VCO.Browser.chrome){str=VCO.Emoji(str)}
if(str.match(/<p>[\s\S]*?<\/p>/)){return str}else{return "<p>"+str+"</p>"}},getParamString:function(obj){var params=[];for(var i in obj){if(obj.hasOwnProperty(i)){params.push(i+"="+obj[i])}}
return "?"+params.join("&")},formatNum:function(num,digits){var pow=Math.pow(10,digits||5);return Math.round(num*pow)/pow},falseFn:function(){return!1},requestAnimFrame:function(){function timeoutDefer(callback){window.setTimeout(callback,1e3/60)}
var requestFn=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||timeoutDefer;return function(callback,context,immediate,contextEl){callback=context?VCO.Util.bind(callback,context):callback;if(immediate&&requestFn===timeoutDefer){callback()}else{requestFn(callback,contextEl)}}}(),bind:function(fn,obj){return function(){return fn.apply(obj,arguments)}},template:function(str,data){return str.replace(/\{ *([\w_]+) *\}/g,function(str,key){var value=data[key];if(!data.hasOwnProperty(key)){throw new Error("No value provided for variable "+str)}
return value})},hexToRgb:function(hex){var shorthandRegex=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;hex=hex.replace(shorthandRegex,function(m,r,g,b){return r+r+g+g+b+b});var result=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);return result?{r:parseInt(result[1],16),g:parseInt(result[2],16),b:parseInt(result[3],16)}:null},ratio:{square:function(size){var s={w:0,h:0};if(size.w>size.h&&size.h>0){s.h=size.h;s.w=size.h}else{s.w=size.w;s.h=size.w}
...
i++}
return ""}else{return ""}},urljoin:function(base_url,path){if(base_url.length&&base_url[base_url.length-1]=="/"){base_url=base_url.substring(0,base_url.length-1)}
if(path.length&&path[0]=="/"){path=path.substring(1)}
var url1=base_url.split("/");var url2=path.split("/");var url3=[];for(var i=0,l=url1.length;i<l;i++){if(url1[i]==".."){url3.pop()}else if(url1[i]=="."){continue}else{url3.push(url1[i])}}
for(var i=0,l=url2.length;i<l;i++){if(url2[i]==".."){url3.pop()}else if(url2[i]=="."){continue}else{url3.push(url2[i])}}
return url3.join("/")},getUrlVars:function(string){var str,vars=[],hash,hashes;str=string.toString();if(str.match("&#038;")){str=str.replace("&#038;","&")}else if(str.match("&#38;")){str=str.replace("&#38;","&")}else if(str.match("&amp;")){str=str.replace("&amp;","&")}
...
return dom};zepto.Z=function(dom,selector){dom=dom||[];dom.__proto__=$.fn;dom.selector=selector||"";return dom};zepto.isZ=function(object){return object instanceof zepto.Z};zepto.init=function(selector,context){var dom;if(!selector)return zepto.Z();else if(typeof selector=="string"){selector=selector.trim();if(selector[0]=="<"&&fragmentRE.test(selector))dom=zepto.fragment(selector,RegExp.$1,context),selector=null;else if(context!==undefined)return $(context).find(selector);else dom=zepto.qsa(document,selector)}else if(isFunction(selector))return $(document).ready(selector);else if(zepto.isZ(selector))return selector;else{if(isArray(selector))dom=compact(selector);else if(isObject(selector))dom=[selector],selector=null;else if(fragmentRE.test(selector))dom=zepto.fragment(selector.trim(),RegExp.$1,context),selector=null;else if(context!==undefined)return $(context).find(selector);else dom=zepto.qsa(document,selector)}
return zepto.Z(dom,selector)};$=function(selector,context){return zepto.init(selector,context)};function extend(target,source,deep){for(key in source)
if(deep&&(isPlainObject(source[key])||isArray(source[key]))){if(isPlainObject(source[key])&&!isPlainObject(target[key]))target[key]={};if(isArray(source[key])&&!isArray(target[key]))target[key]=[];extend(target[key],source[key],deep)}else if(source[key]!==undefined)target[key]=source[key]}
$.extend=function(target){var deep,args=slice.call(arguments,1);if(typeof target=="boolean"){deep=target;target=args.shift()}
args.forEach(function(arg){extend(target,arg,deep)});return target};zepto.qsa=function(element,selector){var found,maybeID=selector[0]=="#",maybeClass=!maybeID&&selector[0]==".",nameOnly=maybeID||maybeClass?selector.slice(1):selector,isSimple=simpleSelectorRE.test(nameOnly);return isDocument(element)&&isSimple&&maybeID?(found=element.getElementById(nameOnly))?[found]:[]:element.nodeType!==1&&element.nodeType!==9?[]:slice.call(isSimple&&!maybeID?maybeClass?element.getElementsByClassName(nameOnly):element.getElementsByTagName(selector):element.querySelectorAll(selector))};function filtered(nodes,selector){return selector==null?$(nodes):$(nodes).filter(selector)}
...
function deserializeValue(value){var num;try{return value?value=="true"||(value=="false"?!1:value=="null"?null:!/^0/.test(value)&&!isNaN(num=Number(value))?num:/^[\[\{]/.test(value)?$.parseJSON(value):value):value}catch(e){return value}}
$.type=type;$.isFunction=isFunction;$.isWindow=isWindow;$.isArray=isArray;$.isPlainObject=isPlainObject;$.isEmptyObject=function(obj){var name;for(name in obj)return!1;return!0};$.inArray=function(elem,array,i){return emptyArray.indexOf.call(array,elem,i)};$.camelCase=camelize;$.trim=function(str){return str==null?"":String.prototype.trim.call(str)};$.uuid=0;$.support={};$.expr={};$.map=function(elements,callback){var value,values=[],i,key;if(likeArray(elements))
for(i=0;i<elements.length;i++){value=callback(elements[i],i);if(value!=null)values.push(value)}else for(key in elements){value=callback(elements[key],key);if(value!=null)values.push(value)}
...
if(!isString(selector)&&!isFunction(callback)&&callback!==!1)callback=data,data=selector,selector=undefined;if(isFunction(data)||data===!1)callback=data,data=undefined;if(callback===!1)callback=returnFalse;return $this.each(function(_,element){if(one)autoRemove=function(e){remove(element,e.type,callback);return callback.apply(this,arguments)};if(selector)delegator=function(e){var evt,match=$(e.target).closest(selector,element).get(0);if(match&&match!==element){evt=$.extend(createProxy(e),{currentTarget:match,liveFired:element});return(autoRemove||callback).apply(match,[evt].concat(slice.call(arguments,1)))}};add(element,event,callback,data,selector,delegator||autoRemove)})};$.fn.off=function(event,selector,callback){var $this=this;if(event&&!isString(event)){$.each(event,function(type,fn){$this.off(type,selector,fn)});return $this}
if(!isString(selector)&&!isFunction(callback)&&callback!==!1)callback=selector,selector=undefined;if(callback===!1)callback=returnFalse;return $this.each(function(){remove(this,event,callback,selector)})};$.fn.trigger=function(event,args){event=isString(event)||$.isPlainObject(event)?$.Event(event):compatible(event);event._args=args;return this.each(function(){if("dispatchEvent" in this)this.dispatchEvent(event);else $(this).triggerHandler(event,args)})};$.fn.triggerHandler=function(event,args){var e,result;this.each(function(i,element){e=createProxy(isString(event)?$.Event(event):event);e._args=args;e.target=element;$.each(findHandlers(element,event.type||event),function(i,handler){result=handler.proxy(e);if(e.isImmediatePropagationStopped())return!1})});return result};("focusin focusout load resize scroll unload click dblclick "+"mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave "+"change select keydown keypress keyup error").split(" ").forEach(function(event){$.fn[event]=function(callback){return callback?this.bind(event,callback):this.trigger(event)}});["focus","blur"].forEach(function(name){$.fn[name]=function(callback){if(callback)this.bind(name,callback);else this.each(function(){try{this[name]()}catch(e){}});return this}});$.Event=function(type,props){if(!isString(type))props=type,type=props.type;var event=document.createEvent(specialEvents[type]||"Events"),bubbles=!0;if(props)
for(var name in props)name=="bubbles"?bubbles=!!props[name]:event[name]=props[name];event.initEvent(type,bubbles,!0);return compatible(event)}})(Zepto);(function($){var jsonpID=0,document=window.document,key,name,rscript=/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,scriptTypeRE=/^(?:text|application)\/javascript/i,xmlTypeRE=/^(?:text|application)\/xml/i,jsonType="application/json",htmlType="text/html",blankRE=/^\s*$/;function triggerAndReturn(context,eventName,data){var event=$.Event(eventName);$(context).trigger(event,data);return!event.isDefaultPrevented()}
...
function ajaxBeforeSend(xhr,settings){var context=settings.context;if(settings.beforeSend.call(context,xhr,settings)===!1||triggerGlobal(settings,context,"ajaxBeforeSend",[xhr,settings])===!1)return!1;triggerGlobal(settings,context,"ajaxSend",[xhr,settings])}
...
window[callbackName]=originalCallback;if(responseData&&$.isFunction(originalCallback))originalCallback(responseData[0]);originalCallback=responseData=undefined});if(ajaxBeforeSend(xhr,options)===!1){abort("abort");return xhr}
window[callbackName]=function(){responseData=arguments};script.src=options.url.replace(/\?(.+)=\?/,"?$1="+callbackName);document.head.appendChild(script);if(options.timeout>0)abortTimeout=setTimeout(function(){abort("timeout")},options.timeout);return xhr};$.ajaxSettings={type:"GET",beforeSend:empty,success:empty,error:empty,complete:empty,context:null,global:!0,xhr:function(){return new window.XMLHttpRequest},accepts:{script:"text/javascript, application/javascript, application/x-javascript",json:jsonType,xml:"application/xml, text/xml",html:htmlType,text:"text/plain"},crossDomain:!1,timeout:0,processData:!0,cache:!0};function mimeToDataType(mime){if(mime)mime=mime.split(";",2)[0];return mime&&(mime==htmlType?"html":mime==jsonType?"json":scriptTypeRE.test(mime)?"script":xmlTypeRE.test(mime)&&"xml")||"text"}
function appendQuery(url,query){if(query=="")return url;return(url+"&"+query).replace(/[&?]{1,2}/,"?")}
function serializeData(options){if(options.processData&&options.data&&$.type(options.data)!="string")options.data=$.param(options.data,options.traditional);if(options.data&&(!options.type||options.type.toUpperCase()=="GET"))options.url=appendQuery(options.url,options.data),options.data=undefined}
$.ajax=function(options){var settings=$.extend({},options||{}),deferred=$.Deferred&&$.Deferred();for(key in $.ajaxSettings)
if(settings[key]===undefined)settings[key]=$.ajaxSettings[key];ajaxStart(settings);if(!settings.crossDomain)settings.crossDomain=/^([\w-]+:)?\/\/([^\/]+)/.test(settings.url)&&RegExp.$2!=window.location.host;if(!settings.url)settings.url=window.location.toString();serializeData(settings);if(settings.cache===!1)settings.url=appendQuery(settings.url,"_="+Date.now());var dataType=settings.dataType,hasPlaceholder=/\?.+=\?/.test(settings.url);if(dataType=="jsonp"||hasPlaceholder){if(!hasPlaceholder)settings.url=appendQuery(settings.url,settings.jsonp?settings.jsonp+"=?":settings.jsonp===!1?"":"callback=?");return $.ajaxJSONP(settings,deferred)}
...
if(settings.contentType||settings.contentType!==!1&&settings.data&&settings.type.toUpperCase()!="GET")setHeader("Content-Type",settings.contentType||"application/x-www-form-urlencoded");if(settings.headers)
for(name in settings.headers)setHeader(name,settings.headers[name]);xhr.setRequestHeader=setHeader;xhr.onreadystatechange=function(){if(xhr.readyState==4){xhr.onreadystatechange=empty;clearTimeout(abortTimeout);var result,error=!1;if(xhr.status>=200&&xhr.status<300||xhr.status==304||xhr.status==0&&protocol=="file:"){dataType=dataType||mimeToDataType(settings.mimeType||xhr.getResponseHeader("content-type"));result=xhr.responseText;try{if(dataType=="script")(1,eval)(result);else if(dataType=="xml")result=xhr.responseXML;else if(dataType=="json")result=blankRE.test(result)?null:$.parseJSON(result)}catch(e){error=e}
if(error)ajaxError(error,"parsererror",xhr,settings,deferred);else ajaxSuccess(result,xhr,settings,deferred)}else{ajaxError(xhr.statusText||null,xhr.status?"error":"abort",xhr,settings,deferred)}}};if(ajaxBeforeSend(xhr,settings)===!1){xhr.abort();ajaxError(null,"abort",xhr,settings,deferred);return xhr}
if(settings.xhrFields)
for(name in settings.xhrFields)xhr[name]=settings.xhrFields[name];var async="async" in settings?settings.async:!0;xhr.open(settings.type,settings.url,async,settings.username,settings.password);for(name in headers)nativeSetHeader.apply(xhr,headers[name]);if(settings.timeout>0)abortTimeout=setTimeout(function(){xhr.onreadystatechange=empty;xhr.abort();ajaxError(null,"timeout",xhr,settings,deferred)},settings.timeout);xhr.send(settings.data?settings.data:null);return xhr};function parseArguments(url,data,success,dataType){var hasData=!$.isFunction(data);return{url:url,data:hasData?data:undefined,success:!hasData?data:$.isFunction(success)?success:undefined,dataType:hasData?dataType||success:success}}
...
$.param=function(obj,traditional){var params=[];params.add=function(k,v){this.push(escape(k)+"="+escape(v))};serialize(params,obj,traditional);return params.join("&").replace(/%20/g,"+")}})(Zepto);(function($){$.fn.serializeArray=function(){var result=[],el;$([].slice.call(this.get(0).elements)).each(function(){el=$(this);var type=el.attr("type");if(this.nodeName.toLowerCase()!="fieldset"&&!this.disabled&&type!="submit"&&type!="reset"&&type!="button"&&(type!="radio"&&type!="checkbox"||this.checked))result.push({name:el.attr("name"),value:el.val()})});return result};$.fn.serialize=function(){var result=[];this.serializeArray().forEach(function(elm){result.push(encodeURIComponent(elm.name)+"="+encodeURIComponent(elm.value))});return result.join("&")};$.fn.submit=function(callback){if(callback)this.bind("submit",callback);else if(this.length){var event=$.Event("submit");this.eq(0).trigger(event);if(!event.isDefaultPrevented())this.get(0).submit()}
return this}})(Zepto);(function($){if(!("__proto__" in{})){$.extend($.zepto,{Z:function(dom,selector){dom=dom||[];$.extend(dom,$.fn);dom.selector=selector||"";dom.__Z=!0;return dom},isZ:function(object){return $.type(object)==="array"&&"__Z" in object}})}
try{getComputedStyle(undefined)}catch(e){var nativeGetComputedStyle=getComputedStyle;window.getComputedStyle=function(element){try{return nativeGetComputedStyle(element)}catch(e){return null}}}})(Zepto);VCO.getJSON=Zepto.getJSON;VCO.ajax=Zepto.ajax})(VCO);VCO.Class=function(){};VCO.Class.extend=function(props){var NewClass=function(){if(this.initialize){this.initialize.apply(this,arguments)}};var F=function(){};F.prototype=this.prototype;var proto=new F;proto.constructor=NewClass;NewClass.prototype=proto;NewClass.superclass=this.prototype;for(var i in this){if(this.hasOwnProperty(i)&&i!=="prototype"&&i!=="superclass"){NewClass[i]=this[i]}}
if(props.statics){VCO.Util.extend(NewClass,props.statics);delete props.statics}
if(props.includes){VCO.Util.extend.apply(null,[proto].concat(props.includes));delete props.includes}
if(props.options&&proto.options){props.options=VCO.Util.extend({},proto.options,props.options)}
VCO.Util.extend(proto,props);NewClass.extend=VCO.Class.extend;NewClass.include=function(props){VCO.Util.extend(this.prototype,props)};return NewClass};VCO.Events={addEventListener:function(type,fn,context){var events=this._vco_events=this._vco_events||{};events[type]=events[type]||[];events[type].push({action:fn,context:context||this});return this},hasEventListeners:function(type){var k="_vco_events";return k in this&&type in this[k]&&this[k][type].length>0},removeEventListener:function(type,fn,context){if(!this.hasEventListeners(type)){return this}
for(var i=0,events=this._vco_events,len=events[type].length;i<len;i++){if(events[type][i].action===fn&&(!context||events[type][i].context===context)){events[type].splice(i,1);return this}}
return this},fireEvent:function(type,data){if(!this.hasEventListeners(type)){return this}
var event=VCO.Util.extend({type:type,target:this},data);var listeners=this._vco_events[type].slice();for(var i=0,len=listeners.length;i<len;i++){listeners[i].action.call(listeners[i].context||this,event)}
return this}};VCO.Events.on=VCO.Events.addEventListener;VCO.Events.off=VCO.Events.removeEventListener;VCO.Events.fire=VCO.Events.fireEvent;(function(){var ua=navigator.userAgent.toLowerCase(),doc=document.documentElement,ie="ActiveXObject" in window,webkit=ua.indexOf("webkit")!==-1,phantomjs=ua.indexOf("phantom")!==-1,android23=ua.search("android [23]")!==-1,mobile=typeof orientation!=="undefined",msPointer=navigator.msPointerEnabled&&navigator.msMaxTouchPoints&&!window.PointerEvent,pointer=window.PointerEvent&&navigator.pointerEnabled&&navigator.maxTouchPoints||msPointer,ie3d=ie&&"transition" in doc.style,webkit3d="WebKitCSSMatrix" in window&&"m11" in new window.WebKitCSSMatrix&&!android23,gecko3d="MozPerspective" in doc.style,opera3d="OTransition" in doc.style,opera=window.opera;var retina="devicePixelRatio" in window&&window.devicePixelRatio>1;if(!retina&&"matchMedia" in window){var matches=window.matchMedia("(min-resolution:144dpi)");retina=matches&&matches.matches}
var touch=!window.L_NO_TOUCH&&!phantomjs&&(pointer||"ontouchstart" in window||window.DocumentTouch&&document instanceof window.DocumentTouch);VCO.Browser={ie:ie,ielt9:ie&&!document.addEventListener,webkit:webkit,firefox:ua.indexOf("gecko")!==-1&&!webkit&&!window.opera&&!ie,android:ua.indexOf("android")!==-1,android23:android23,chrome:ua.indexOf("chrome")!==-1,ie3d:ie3d,webkit3d:webkit3d,gecko3d:gecko3d,opera3d:opera3d,any3d:!window.L_DISABLE_3D&&(ie3d||webkit3d||gecko3d||opera3d)&&!phantomjs,mobile:mobile,mobileWebkit:mobile&&webkit,mobileWebkit3d:mobile&&webkit3d,mobileOpera:mobile&&window.opera,touch:!!touch,msPointer:!!msPointer,pointer:!!pointer,retina:!!retina,orientation:function(){var w=window.innerWidth,h=window.innerHeight,_orientation="portrait";if(w>h){_orientation="landscape"}
if(Math.abs(window.orientation)==90){}
trace(_orientation);return _orientation}}})();VCO.Load=function(doc){var loaded=[];function isLoaded(url){var i=0,has_loaded=!1;for(i=0;i<loaded.length;i++){if(loaded[i]==url){has_loaded=!0}}
...
function tween(duration,fn,done,ease,from,to){ease=fun(ease)?ease:morpheus.easings[ease]||nativeTween;var time=duration||thousand,self=this,diff=to-from,start=now(),stop=0,end=0;function run(t){var delta=t-start;if(delta>time||stop){to=isFinite(to)?to:1;stop?end&&fn(to):fn(to);die(run);return done&&done.apply(self)}
isFinite(to)?fn(diff*ease(delta/time)+from):fn(ease(delta/time))}
live(run);return{stop:function(jump){stop=1;end=jump;if(!jump)done=null}}}
function bezier(points,pos){var n=points.length,r=[],i,j;for(i=0;i<n;++i){r[i]=[points[i][0],points[i][1]]}
for(j=1;j<n;++j){for(i=0;i<n-j;++i){r[i][0]=(1-pos)*r[i][0]+pos*r[parseInt(i+1,10)][0];r[i][1]=(1-pos)*r[i][1]+pos*r[parseInt(i+1,10)][1]}}
return[r[0][0],r[0][1]]}
function nextColor(pos,start,finish){var r=[],i,e,from,to;for(i=0;i<6;i++){from=Math.min(15,parseInt(start.charAt(i),16));to=Math.min(15,parseInt(finish.charAt(i),16));e=Math.floor((to-from)*pos+from);e=e>15?15:e<0?0:e;r[i]=e.toString(16)}
return "#"+r.join("")}
function getTweenVal(pos,units,begin,end,k,i,v){if(k=="transform"){v={};for(var t in begin[i][k]){v[t]=t in end[i][k]?Math.round(((end[i][k][t]-begin[i][k][t])*pos+begin[i][k][t])*thousand)/thousand:begin[i][k][t]}
return v}else if(typeof begin[i][k]=="string"){return nextColor(pos,begin[i][k],end[i][k])}else{v=Math.round(((end[i][k]-begin[i][k])*pos+begin[i][k])*thousand)/thousand;if(!(k in unitless))v+=units[i][k]||"px";return v}}
function by(val,start,m,r,i){return(m=relVal.exec(val))?(i=parseFloat(m[2]))&&start+(m[1]=="+"?1:-1)*i:parseFloat(val)}
function morpheus(elements,options){var els=elements?els=isFinite(elements.length)?elements:[elements]:[],i,complete=options.complete,duration=options.duration,ease=options.easing,points=options.bezier,begin=[],end=[],units=[],bez=[],originalLeft,originalTop;if(points){originalLeft=options.left;originalTop=options.top;delete options.right;delete options.bottom;delete options.left;delete options.top}
for(i=els.length;i--;){begin[i]={};end[i]={};units[i]={};if(points){var left=getStyle(els[i],"left"),top=getStyle(els[i],"top"),xy=[by(fun(originalLeft)?originalLeft(els[i]):originalLeft||0,parseFloat(left)),by(fun(originalTop)?originalTop(els[i]):originalTop||0,parseFloat(top))];bez[i]=fun(points)?points(els[i],xy):points;bez[i].push(xy);bez[i].unshift([parseInt(left,10),parseInt(top,10)])}
for(var k in options){switch(k){case "complete":case "duration":case "easing":case "bezier":continue}
var v=getStyle(els[i],k),unit,tmp=fun(options[k])?options[k](els[i]):options[k];if(typeof tmp=="string"&&rgbOhex.test(tmp)&&!rgbOhex.test(v)){delete options[k];continue}
begin[i][k]=k=="transform"?parseTransform(v):typeof tmp=="string"&&rgbOhex.test(tmp)?toHex(v).slice(1):parseFloat(v);end[i][k]=k=="transform"?parseTransform(tmp,begin[i][k]):typeof tmp=="string"&&tmp.charAt(0)=="#"?toHex(tmp).slice(1):by(tmp,parseFloat(v));typeof tmp=="string"&&(unit=tmp.match(numUnit))&&(units[i][k]=unit[1])}}
return tween.apply(els,[duration,function(pos,v,xy){for(i=els.length;i--;){if(points){xy=bezier(bez[i],pos);els[i].style.left=xy[0]+"px";els[i].style.top=xy[1]+"px"}
for(var k in options){v=getTweenVal(pos,units,begin,end,k,i);k=="transform"?els[i].style[transform]=formatTransform(v):k=="opacity"&&!opacity?els[i].style.filter="alpha(opacity="+v*100+")":els[i].style[camelize(k)]=v}}},complete,ease])}
morpheus.tween=tween;morpheus.getStyle=getStyle;morpheus.bezier=bezier;morpheus.transform=transform;morpheus.parseTransform=parseTransform;morpheus.formatTransform=formatTransform;morpheus.easings={};return morpheus}();VCO.Point=function(x,y,round){this.x=round?Math.round(x):x;this.y=round?Math.round(y):y};VCO.Point.prototype={add:function(point){return this.clone()._add(point)},_add:function(point){this.x+=point.x;this.y+=point.y;return this},subtract:function(point){return this.clone()._subtract(point)},_subtract:function(point){this.x-=point.x;this.y-=point.y;return this},divideBy:function(num,round){return new VCO.Point(this.x/num,this.y/num,round)},multiplyBy:function(num){return new VCO.Point(this.x*num,this.y*num)},distanceTo:function(point){var x=point.x-this.x,y=point.y-this.y;return Math.sqrt(x*x+y*y)},round:function(){return this.clone()._round()},_round:function(){this.x=Math.round(this.x);this.y=Math.round(this.y);return this},clone:function(){return new VCO.Point(this.x,this.y)},toString:function(){return "Point("+VCO.Util.formatNum(this.x)+", "+VCO.Util.formatNum(this.y)+")"}};VCO.DomMixins={show:function(animate){if(animate){}else{this._el.container.style.display="block"}},hide:function(animate){this._el.container.style.display="none"},addTo:function(container){container.appendChild(this._el.container);this.onAdd()},removeFrom:function(container){container.removeChild(this._el.container);this.onRemove()},animatePosition:function(pos,el,use_percent){var ani={duration:this.options.duration,easing:this.options.ease};for(var name in pos){if(pos.hasOwnProperty(name)){if(use_percent){ani[name]=pos[name]+"%"}else{ani[name]=pos[name]+"px"}}}
if(this.animator){this.animator.stop()}
this.animator=VCO.Animate(el,ani)},onLoaded:function(){this.fire("loaded",this.data)},onAdd:function(){this.fire("added",this.data)},onRemove:function(){this.fire("removed",this.data)},setPosition:function(pos,el){for(var name in pos){if(pos.hasOwnProperty(name)){if(el){el.style[name]=pos[name]+"px"}else{this._el.container.style[name]=pos[name]+"px"}}}},getPosition:function(){return VCO.Dom.getPosition(this._el.container)}};VCO.Dom={get:function(id){return typeof id==="string"?document.getElementById(id):id},getByClass:function(id){if(id){return document.getElementsByClassName(id)}},create:function(tagName,className,container){var el=document.createElement(tagName);el.className=className;if(container){container.appendChild(el)}
...
return!1}};VCO.Util.extend(VCO.Dom,{TRANSITION:VCO.Dom.testProp(["transition","webkitTransition","OTransition","MozTransition","msTransition"]),TRANSFORM:VCO.Dom.testProp(["transformProperty","WebkitTransform","OTransform","MozTransform","msTransform"]),TRANSLATE_OPEN:"translate"+(VCO.Browser.webkit3d?"3d(":"("),TRANSLATE_CLOSE:VCO.Browser.webkit3d?",0)":")"});VCO.DomUtil={get:function(id){return typeof id==="string"?document.getElementById(id):id},getStyle:function(el,style){var value=el.style[style];if(!value&&el.currentStyle){value=el.currentStyle[style]}
if(!value||value==="auto"){var css=document.defaultView.getComputedStyle(el,null);value=css?css[style]:null}
return value==="auto"?null:value},getViewportOffset:function(element){var top=0,left=0,el=element,docBody=document.body;do{top+=el.offsetTop||0;left+=el.offsetLeft||0;if(el.offsetParent===docBody&&VCO.DomUtil.getStyle(el,"position")==="absolute"){break}
el=el.offsetParent}while(el);el=element;do{if(el===docBody){break}
top-=el.scrollTop||0;left-=el.scrollLeft||0;el=el.parentNode}while(el);return new VCO.Point(left,top)},create:function(tagName,className,container){var el=document.createElement(tagName);el.className=className;if(container){container.appendChild(el)}
return el},disableTextSelection:function(){if(document.selection&&document.selection.empty){document.selection.empty()}
if(!this._onselectstart){this._onselectstart=document.onselectstart;document.onselectstart=VCO.Util.falseFn}},enableTextSelection:function(){document.onselectstart=this._onselectstart;this._onselectstart=null},hasClass:function(el,name){return el.className.length>0&&new RegExp("(^|\\s)"+name+"(\\s|$)").test(el.className)},addClass:function(el,name){if(!VCO.DomUtil.hasClass(el,name)){el.className+=(el.className?" ":"")+name}},removeClass:function(el,name){el.className=el.className.replace(/(\S+)\s*/g,function(w,match){if(match===name){return ""}
return w}).replace(/^\s+/,"")},setOpacity:function(el,value){if(VCO.Browser.ie){el.style.filter="alpha(opacity="+Math.round(value*100)+")"}else{el.style.opacity=value}},testProp:function(props){var style=document.documentElement.style;for(var i=0;i<props.length;i++){if(props[i]in style){return props[i]}}
return!1},getTranslateString:function(point){return VCO.DomUtil.TRANSLATE_OPEN+point.x+"px,"+point.y+"px"+VCO.DomUtil.TRANSLATE_CLOSE},getScaleString:function(scale,origin){var preTranslateStr=VCO.DomUtil.getTranslateString(origin),scaleStr=" scale("+scale+") ",postTranslateStr=VCO.DomUtil.getTranslateString(origin.multiplyBy(-1));return preTranslateStr+scaleStr+postTranslateStr},setPosition:function(el,point){el._vco_pos=point;if(VCO.Browser.webkit3d){el.style[VCO.DomUtil.TRANSFORM]=VCO.DomUtil.getTranslateString(point);if(VCO.Browser.android){el.style["-webkit-perspective"]="1000";el.style["-webkit-backface-visibility"]="hidden"}}else{el.style.left=point.x+"px";el.style.top=point.y+"px"}},getPosition:function(el){return el._vco_pos}};VCO.DomEvent={addListener:function(obj,type,fn,context){var id=VCO.Util.stamp(fn),key="_vco_"+type+id;if(obj[key]){return}
var handler=function(e){return fn.call(context||obj,e||VCO.DomEvent._getEvent())};if(VCO.Browser.touch&&type==="dblclick"&&this.addDoubleTapListener){this.addDoubleTapListener(obj,handler,id)}else if("addEventListener" in obj){if(type==="mousewheel"){obj.addEventListener("DOMMouseScroll",handler,!1);obj.addEventListener(type,handler,!1)}else if(type==="mouseenter"||type==="mouseleave"){var originalHandler=handler,newType=type==="mouseenter"?"mouseover":"mouseout";handler=function(e){if(!VCO.DomEvent._checkMouse(obj,e)){return}
...
obj[key]=handler},removeListener:function(obj,type,fn){var id=VCO.Util.stamp(fn),key="_vco_"+type+id,handler=obj[key];if(!handler){return}
...
return e},stopPropagation:function(e){if(e.stopPropagation){e.stopPropagation()}else{e.cancelBubble=!0}},disableClickPropagation:function(el){VCO.DomEvent.addListener(el,VCO.Draggable.START,VCO.DomEvent.stopPropagation);VCO.DomEvent.addListener(el,"click",VCO.DomEvent.stopPropagation);VCO.DomEvent.addListener(el,"dblclick",VCO.DomEvent.stopPropagation)},preventDefault:function(e){if(e.preventDefault){e.preventDefault()}else{e.returnValue=!1}},stop:function(e){VCO.DomEvent.preventDefault(e);VCO.DomEvent.stopPropagation(e)},getWheelDelta:function(e){var delta=0;if(e.wheelDelta){delta=e.wheelDelta/120}
if(e.detail){delta=-e.detail/3}
return delta}};VCO.Draggable=VCO.Class.extend({includes:VCO.Events,_el:{},mousedrag:{down:"mousedown",up:"mouseup",leave:"mouseleave",move:"mousemove"},touchdrag:{down:"touchstart",up:"touchend",leave:"mouseleave",move:"touchmove"},initialize:function(drag_elem,options,move_elem){this._el={drag:drag_elem,move:drag_elem};if(move_elem){this._el.move=move_elem}
this.options={enable:{x:!0,y:!0},constraint:{top:!1,bottom:!1,left:!1,right:!1},momentum_multiplier:2e3,duration:1e3,ease:VCO.Ease.easeInOutQuint};this.animator=null;this.dragevent=this.mousedrag;if(VCO.Browser.touch){this.dragevent=this.touchdrag}
this.data={sliding:!1,direction:"none",pagex:{start:0,end:0},pagey:{start:0,end:0},pos:{start:{x:0,y:0},end:{x:0,y:0}},new_pos:{x:0,y:0},new_pos_parent:{x:0,y:0},time:{start:0,end:0},touch:!1};VCO.Util.mergeData(this.options,options)},enable:function(e){this.data.pos.start=0;this._el.move.style.left=this.data.pos.start.x+"px";this._el.move.style.top=this.data.pos.start.y+"px";this._el.move.style.position="absolute"},disable:function(){VCO.DomEvent.removeListener(this._el.drag,this.dragevent.down,this._onDragStart,this);VCO.DomEvent.removeListener(this._el.drag,this.dragevent.up,this._onDragEnd,this)},stopMomentum:function(){if(this.animator){this.animator.stop()}},updateConstraint:function(c){this.options.constraint=c},_onDragStart:function(e){if(VCO.Browser.touch){if(e.originalEvent){this.data.pagex.start=e.originalEvent.touches[0].screenX;this.data.pagey.start=e.originalEvent.touches[0].screenY}else{this.data.pagex.start=e.targetTouches[0].screenX;this.data.pagey.start=e.targetTouches[0].screenY}}else{this.data.pagex.start=e.pageX;this.data.pagey.start=e.pageY}
...
this.data.pos.start=VCO.Dom.getPosition(this._el.drag);this.data.time.start=(new Date).getTime();this.fire("dragstart",this.data);VCO.DomEvent.addListener(this._el.drag,this.dragevent.move,this._onDragMove,this);VCO.DomEvent.addListener(this._el.drag,this.dragevent.leave,this._onDragEnd,this)},_onDragEnd:function(e){this.data.sliding=!1;VCO.DomEvent.removeListener(this._el.drag,this.dragevent.move,this._onDragMove,this);VCO.DomEvent.removeListener(this._el.drag,this.dragevent.leave,this._onDragEnd,this);this.fire("dragend",this.data);this._momentum()},_onDragMove:function(e){e.preventDefault();this.data.sliding=!0;if(VCO.Browser.touch){if(e.originalEvent){this.data.pagex.end=e.originalEvent.touches[0].screenX;this.data.pagey.end=e.originalEvent.touches[0].screenY}else{this.data.pagex.end=e.targetTouches[0].screenX;this.data.pagey.end=e.targetTouches[0].screenY}}else{this.data.pagex.end=e.pageX;this.data.pagey.end=e.pageY}
this.data.pos.end=VCO.Dom.getPosition(this._el.drag);this.data.new_pos.x=-(this.data.pagex.start-this.data.pagex.end-this.data.pos.start.x);this.data.new_pos.y=-(this.data.pagey.start-this.data.pagey.end-this.data.pos.start.y);if(this.options.enable.x){this._el.move.style.left=this.data.new_pos.x+"px"}
if(this.options.enable.y){this._el.move.style.top=this.data.new_pos.y+"px"}
this.fire("dragmove",this.data)},_momentum:function(){var pos_adjust={x:0,y:0,time:0},pos_change={x:0,y:0,time:0},swipe=!1,swipe_direction="";if(VCO.Browser.touch){}
...
if(pos_change.time<3e3){swipe=!0}
if(Math.abs(pos_change.x)>1e4){this.data.direction="left";if(pos_change.x>0){this.data.direction="right"}}
if(Math.abs(pos_change.y)>1e4){this.data.direction="up";if(pos_change.y>0){this.data.direction="down"}}
...
this.animator=VCO.Animate(this._el.move,animate);this.fire("momentum",this.data)}});VCO.Swipable=VCO.Class.extend({includes:VCO.Events,_el:{},mousedrag:{down:"mousedown",up:"mouseup",leave:"mouseleave",move:"mousemove"},touchdrag:{down:"touchstart",up:"touchend",leave:"mouseleave",move:"touchmove"},initialize:function(drag_elem,move_elem,options){this._el={drag:drag_elem,move:drag_elem};if(move_elem){this._el.move=move_elem}
this.options={snap:!1,enable:{x:!0,y:!0},constraint:{top:!1,bottom:!1,left:0,right:!1},momentum_multiplier:2e3,duration:1e3,ease:VCO.Ease.easeInOutQuint};this.animator=null;this.dragevent=this.mousedrag;if(VCO.Browser.touch){this.dragevent=this.touchdrag}
this.data={sliding:!1,direction:"none",pagex:{start:0,end:0},pagey:{start:0,end:0},pos:{start:{x:0,y:0},end:{x:0,y:0}},new_pos:{x:0,y:0},new_pos_parent:{x:0,y:0},time:{start:0,end:0},touch:!1};VCO.Util.mergeData(this.options,options)},enable:function(e){VCO.DomEvent.addListener(this._el.drag,this.dragevent.down,this._onDragStart,this);VCO.DomEvent.addListener(this._el.drag,this.dragevent.up,this._onDragEnd,this);this.data.pos.start=0;this._el.move.style.left=this.data.pos.start.x+"px";this._el.move.style.top=this.data.pos.start.y+"px";this._el.move.style.position="absolute"},disable:function(){VCO.DomEvent.removeListener(this._el.drag,this.dragevent.down,this._onDragStart,this);VCO.DomEvent.removeListener(this._el.drag,this.dragevent.up,this._onDragEnd,this)},stopMomentum:function(){if(this.animator){this.animator.stop()}},updateConstraint:function(c){this.options.constraint=c},_onDragStart:function(e){if(this.animator){this.animator.stop()}
if(VCO.Browser.touch){if(e.originalEvent){this.data.pagex.start=e.originalEvent.touches[0].screenX;this.data.pagey.start=e.originalEvent.touches[0].screenY}else{this.data.pagex.start=e.targetTouches[0].screenX;this.data.pagey.start=e.targetTouches[0].screenY}}else{this.data.pagex.start=e.pageX;this.data.pagey.start=e.pageY}
if(this.options.enable.x){}
if(this.options.enable.y){}
this.data.pos.start={x:this._el.move.offsetLeft,y:this._el.move.offsetTop};this.data.time.start=(new Date).getTime();this.fire("dragstart",this.data);VCO.DomEvent.addListener(this._el.drag,this.dragevent.move,this._onDragMove,this);VCO.DomEvent.addListener(this._el.drag,this.dragevent.leave,this._onDragEnd,this)},_onDragEnd:function(e){this.data.sliding=!1;VCO.DomEvent.removeListener(this._el.drag,this.dragevent.move,this._onDragMove,this);VCO.DomEvent.removeListener(this._el.drag,this.dragevent.leave,this._onDragEnd,this);this.fire("dragend",this.data);this._momentum()},_onDragMove:function(e){var change={x:0,y:0};this.data.sliding=!0;if(VCO.Browser.touch){if(e.originalEvent){this.data.pagex.end=e.originalEvent.touches[0].screenX;this.data.pagey.end=e.originalEvent.touches[0].screenY}else{this.data.pagex.end=e.targetTouches[0].screenX;this.data.pagey.end=e.targetTouches[0].screenY}}else{this.data.pagex.end=e.pageX;this.data.pagey.end=e.pageY}
...
if(!this.options.enable.y){this.data.new_pos.y=this.data.pos.start.y}else if(this.data.new_pos.y<0){this.data.new_pos.y=0}
if(pos_change.time<2e3){swipe=!0}
...
this._el.content=VCO.Dom.create("div","vco-media-content",this._el.link)}else{this._el.content=VCO.Dom.create("div","vco-media-content",this._el.content_container)}},_updateDisplay:function(w,h,l){if(w){this.options.width=w}
...
if(this.data.text&&this.data.text.headline){this.has.headline=!0;this.title=this.data.text.headline}
if(this.has.media){this.data.media.mediatype=VCO.MediaType(this.data.media);this.options.media_name=this.data.media.mediatype.name;this.options.media_type=this.data.media.mediatype.type;this._media=new this.data.media.mediatype.cls(this.data.media,this.options)}
if(this.has.text||this.has.headline){this._text=new VCO.Media.Text(this.data.text,{title:this.has.title})}
if(!this.has.text&&!this.has.headline&&this.has.media){this._el.container.className+=" vco-slide-media-only";this._media.addTo(this._el.content)}else if(this.has.headline&&this.has.media&&!this.has.text){this._el.container.className+=" vco-slide-media-only";this._text.addTo(this._el.content);this._media.addTo(this._el.content)}else if(this.has.text&&this.has.media){this._media.addTo(this._el.content);this._text.addTo(this._el.content)}else if(this.has.text||this.has.headline){this._el.container.className+=" vco-slide-text-only";this._text.addTo(this._el.content)}
this.onLoaded()},_initEvents:function(){},_updateDisplay:function(width,height,layout){var pad_left,pad_right,new_width;if(width){this.options.width=width}else{this.options.width=this._el.container.offsetWidth}
if(VCO.Browser.mobile&&this.options.width<=this.options.skinny_size){pad_left=0+"px";pad_right=0+"px";new_width=this.options.width-0+"px"}else if(layout=="landscape"){pad_left=40+"px";pad_right=75+"px";new_width=this.options.width-(75+40)+"px"}else if(this.options.width<=this.options.skinny_size){pad_left=this.options.slide_padding_lr+"px";pad_right=this.options.slide_padding_lr+"px";new_width=this.options.width-this.options.slide_padding_lr*2+"px"}else{pad_left=this.options.slide_padding_lr+"px";pad_right=this.options.slide_padding_lr+"px";new_width=this.options.width-this.options.slide_padding_lr*2+"px"}
this._el.content.style.paddingLeft=pad_left;this._el.content.style.paddingRight=pad_right;this._el.content.style.width=new_width;if(this._el.call_to_action){this._el.call_to_action.style.paddingLeft=pad_left;this._el.call_to_action.style.paddingRight=pad_right;this._el.call_to_action.style.width=new_width}
if(height){this.options.height=height}else{this.options.height=this._el.container.offsetHeight}
if(this._media){if(!this.has.text&&this.has.headline){this._media.updateDisplay(this.options.width,this.options.height-this._text.headlineHeight(),layout)}else{this._media.updateDisplay(this.options.width,this.options.height,layout)}}}});VCO.SlideNav=VCO.Class.extend({includes:[VCO.Events,VCO.DomMixins],_el:{},initialize:function(data,options,add_to_container){this._el={container:{},content_container:{},icon:{},title:{},description:{}};this.mediatype={};this.data={title:"Navigation",description:"Description"};this.options={direction:"previous"};this.animator=null;this.animator_position=null;VCO.Util.mergeData(this.options,options);VCO.Util.mergeData(this.data,data);this._el.container=VCO.Dom.create("div","vco-slidenav-"+this.options.direction);if(VCO.Browser.mobile){this._el.container.setAttribute("ontouchstart"," ")}
this._initLayout();this._initEvents();if(add_to_container){add_to_container.appendChild(this._el.container)}},update:function(d){this._update(d)},setColor:function(inverted){if(inverted){this._el.content_container.className="vco-slidenav-content-container vco-slidenav-inverted"}else{this._el.content_container.className="vco-slidenav-content-container"}},updatePosition:function(pos,use_percent,duration,ease,start_value,return_to_default){var self=this,ani={duration:duration,easing:ease,complete:function(){self._onUpdatePositionComplete(return_to_default)}};var _start_value=start_value;for(var name in pos){if(pos.hasOwnProperty(name)){if(use_percent){ani[name]=pos[name]+"%"}else{ani[name]=pos[name]+"px"}}}
if(this.animator_position){this.animator_position.stop()}
var prop_to_set;if(ani.right){prop_to_set="right"}else{prop_to_set="left"}
if(use_percent){this._el.container.style[prop_to_set]=_start_value+"%"}else{this._el.container.style[prop_to_set]=_start_value+"px"}
this.animator_position=VCO.Animate(this._el.container,ani)},_onUpdatePositionComplete:function(return_to_default){if(return_to_default){this._el.container.style.left="";this._el.container.style.right=""}},_onMouseClick:function(){this.fire("clicked",this.options)},_update:function(d){this.data=VCO.Util.mergeData(this.data,d);if(this.data.title!=""){this._el.title.innerHTML=this.data.title}
if(this.data.date!=""){this._el.description.innerHTML=this.data.description}},_initLayout:function(){this._el.content_container=VCO.Dom.create("div","vco-slidenav-content-container",this._el.container);this._el.icon=VCO.Dom.create("div","vco-slidenav-icon",this._el.content_container);this._el.title=VCO.Dom.create("div","vco-slidenav-title",this._el.content_container);this._el.description=VCO.Dom.create("div","vco-slidenav-description",this._el.content_container);this._el.icon.innerHTML="&nbsp;";this._update()},_initEvents:function(){VCO.DomEvent.addListener(this._el.container,"click",this._onMouseClick,this)}});VCO.StorySlider=VCO.Class.extend({includes:VCO.Events,initialize:function(elem,data,options,init){this._el={container:{},background:{},slider_container_mask:{},slider_container:{},slider_item_container:{}};this._nav={};this._nav.previous={};this._nav.next={};this.slide_spacing=0;this._slides=[];this._swipable;this.preloadTimer;this._message;this.current_slide=0;this.current_bg_color=null;this.data={};this.options={id:"",layout:"portrait",width:600,height:600,default_bg_color:{r:256,g:256,b:256},slide_padding_lr:40,start_at_slide:1,slide_default_fade:"0%",duration:1e3,ease:VCO.Ease.easeInOutQuint,dragging:!0,trackResize:!0};if(typeof elem==="object"){this._el.container=elem;this.options.id=VCO.Util.unique_ID(6,"vco")}else{this.options.id=elem;this._el.container=VCO.Dom.get(elem)}
if(!this._el.container.id){this._el.container.id=this.options.id}
this.animator=null;this.animator_background=null;VCO.Util.mergeData(this.options,options);VCO.Util.mergeData(this.data,data);if(init){this.init()}},init:function(){this._initLayout();this._initEvents();this._initData();this._updateDisplay();this.goTo(this.options.start_at_slide);this._onLoaded();this._introInterface()},updateDisplay:function(w,h,a,l){this._updateDisplay(w,h,a,l)},createSlide:function(d){this._createSlide(d)},createSlides:function(array){this._createSlides(array)},_createSlides:function(array){for(var i=0;i<array.length;i++){if(array[i].uniqueid==""){array[i].uniqueid=VCO.Util.unique_ID(6,"vco-slide")}
if(i==0){this._createSlide(array[i],!0)}else{this._createSlide(array[i],!1)}}},_createSlide:function(d,title_slide){var slide=new VCO.Slide(d,this.options,title_slide);this._addSlide(slide);this._slides.push(slide)},_destroySlide:function(slide){this._removeSlide(slide);for(var i=0;i<this._slides.length;i++){if(this._slides[i]==slide){this._slides.splice(i,1)}}},_addSlide:function(slide){slide.addTo(this._el.slider_item_container);slide.on("added",this._onSlideAdded,this);slide.on("background_change",this._onBackgroundChange,this)},_removeSlide:function(slide){slide.removeFrom(this._el.slider_item_container);slide.off("added",this._onSlideAdded,this);slide.off("background_change",this._onBackgroundChange)},goToId:function(n,fast,displayupdate){if(typeof n=="string"||n instanceof String){_n=VCO.Util.findArrayNumberByUniqueID(n,this._slides,"uniqueid")}else{_n=n}
this.goTo(_n,fast,displayupdate)},goTo:function(n,fast,displayupdate){var self=this;this.changeBackground({color_value:"",image:!1});if(this.preloadTimer){clearTimeout(this.preloadTimer)}
for(var i=0;i<this._slides.length;i++){this._slides[i].setActive(!1)}
if(n<this._slides.length&&n>=0){this.current_slide=n;if(this.animator){this.animator.stop()}
if(this._swipable){this._swipable.stopMomentum()}
if(fast){this._el.slider_container.style.left=-(this.slide_spacing*n)+"px";this._onSlideChange(displayupdate)}else{this.animator=VCO.Animate(this._el.slider_container,{left:-(this.slide_spacing*n)+"px",duration:this.options.duration,easing:this.options.ease,complete:this._onSlideChange(displayupdate)})}
this._slides[this.current_slide].setActive(!0);if(this._slides[this.current_slide+1]){this.showNav(this._nav.next,!0);this._nav.next.update(this.getNavInfo(this._slides[this.current_slide+1]))}else{this.showNav(this._nav.next,!1)}
if(this._slides[this.current_slide-1]){this.showNav(this._nav.previous,!0);this._nav.previous.update(this.getNavInfo(this._slides[this.current_slide-1]))}else{this.showNav(this._nav.previous,!1)}
this.preloadTimer=setTimeout(function(){self.preloadSlides()},this.options.duration)}},preloadSlides:function(){if(this._slides[this.current_slide+1]){this._slides[this.current_slide+1].loadMedia();this._slides[this.current_slide+1].scrollToTop()}
if(this._slides[this.current_slide+2]){this._slides[this.current_slide+2].loadMedia();this._slides[this.current_slide+2].scrollToTop()}
if(this._slides[this.current_slide-1]){this._slides[this.current_slide-1].loadMedia();this._slides[this.current_slide-1].scrollToTop()}
if(this._slides[this.current_slide-2]){this._slides[this.current_slide-2].loadMedia();this._slides[this.current_slide-2].scrollToTop()}},getNavInfo:function(slide){var n={title:"",description:""};if(slide.data.text){if(slide.data.text.headline){n.title=slide.data.text.headline}}
return n},next:function(){if(this.current_slide+1<this._slides.length){this.goTo(this.current_slide+1)}else{this.goTo(this.current_slide)}},previous:function(){if(this.current_slide-1>=0){this.goTo(this.current_slide-1)}else{this.goTo(this.current_slide)}},showNav:function(nav_obj,show){if(this.options.width<=500&&VCO.Browser.mobile){}else{if(show){nav_obj.show()}else{nav_obj.hide()}}},changeBackground:function(bg){var self=this,do_animation=!1;var bg_color={r:256,g:256,b:256},bg_color_rgb,bg_percent_start=this.options.slide_default_fade,bg_percent_end="15%",bg_alpha_end="0.87",bg_css="",bg_old=this._el.background.getAttribute("style");if(bg.color_value){bg_color=VCO.Util.hexToRgb(bg.color_value)}else{bg_color=this.options.default_bg_color}
if(this.animator_background){this.animator_background.stop()}
bg_color_rgb=bg_color.r+","+bg_color.g+","+bg_color.b;if(!this.current_bg_color||this.current_bg_color!=bg_color_rgb){this.current_bg_color=bg_color_rgb;do_animation=!0}
if(do_animation){if(this.options.layout=="landscape"){this._nav.next.setColor(!1);this._nav.previous.setColor(!1);if(bg_color.r<255&&bg_color.g<255&&bg_color.b<255){bg_percent_start="15%"}
if(bg.image){bg_percent_start="0%"}
bg_css+="opacity:0;";bg_css+="background-image: -webkit-linear-gradient(left, color-stop(rgba("+bg_color_rgb+",0.0001 ) "+bg_percent_start+"), color-stop(rgba("+bg_color_rgb+","+bg_alpha_end+") "+bg_percent_end+"));";bg_css+="background-image: linear-gradient(to right, rgba("+bg_color_rgb+",0.0001 ) "+bg_percent_start+", rgba("+bg_color_rgb+","+bg_alpha_end+") "+bg_percent_end+");";bg_css+="background-repeat: repeat-x;";bg_css+="filter: e(%('progid:DXImageTransform.Microsoft.gradient(startColorstr='%d', endColorstr='%d', GradientType=1)',argb("+bg_color_rgb+", 0.0001),argb("+bg_color_rgb+",0.80)));"}else{if(bg.color_value){bg_css+="background-color:"+bg.color_value+";"}else{bg_css+="background-color:#FFF;"}
if(bg_color.r<255&&bg_color.g<255&&bg_color.b<255||bg.image){this._nav.next.setColor(!0);this._nav.previous.setColor(!0)}else{this._nav.next.setColor(!1);this._nav.previous.setColor(!1)}}
this.animator_background=VCO.Animate(this._el.background,{opacity:0,duration:this.options.duration/2,easing:this.options.ease,complete:function(){self.fadeInBackground(bg_css)}})}},fadeInBackground:function(bg_css){if(this.animator_background){this.animator_background.stop()}
if(bg_css){this._el.background.setAttribute("style",bg_css)}
this.animator_background=VCO.Animate(this._el.background,{opacity:1,duration:this.options.duration/2,easing:this.options.ease})},_updateDisplay:function(width,height,animate,layout){var nav_pos,_layout;if(typeof layout==="undefined"){_layout=this.options.layout}else{_layout=layout}
...
this.lat=lat;this.lng=lng;if(alt!==undefined){this.alt=parseFloat(alt)}};L.extend(L.LatLng,{DEG_TO_RAD:Math.PI/180,RAD_TO_DEG:180/Math.PI,MAX_MARGIN:1e-9});L.LatLng.prototype={equals:function(obj){if(!obj){return!1}
...
this._tileLayersToLoad=this._tileLayersNum;var loading=!this._loaded;this._loaded=!0;if(loading){this.fire("load");this.eachLayer(this._layerAdd,this)}
this.fire("viewreset",{hard:!preserveMapOffset});this.fire("move");if(zoomChanged||afterZoomAnim){this.fire("zoomend")}
...
return related!==el},_getEvent:function(){var e=window.event;if(!e){var caller=arguments.callee.caller;while(caller){e=caller["arguments"][0];if(e&&window.Event===e.constructor){break}
caller=caller.caller}}
return e},_filterClick:function(e,handler){var timeStamp=e.timeStamp||e.originalEvent.timeStamp,elapsed=L.DomEvent._lastClick&&timeStamp-L.DomEvent._lastClick;if(elapsed&&elapsed>100&&elapsed<1e3||e.target._simulatedClick&&!e._simulated){L.DomEvent.stop(e);return}
...
L.DomEvent.preventDefault(e);if(!this._moved){this.fire("dragstart");this._moved=!0;this._startPos=L.DomUtil.getPosition(this._element).subtract(offset);L.DomUtil.addClass(document.body,"leaflet-dragging");L.DomUtil.addClass(e.target||e.srcElement,"leaflet-drag-target")}
this._newPos=this._startPos.add(offset);this._moving=!0;L.Util.cancelAnimFrame(this._animRequest);this._animRequest=L.Util.requestAnimFrame(this._updatePosition,this,!0,this._dragStartTarget)},_updatePosition:function(){this.fire("predrag");L.DomUtil.setPosition(this._element,this._newPos);this.fire("drag")},_onUp:function(e){L.DomUtil.removeClass(document.body,"leaflet-dragging");L.DomUtil.removeClass(e.target||e.srcElement,"leaflet-drag-target");for(var i in L.Draggable.MOVE){L.DomEvent.off(document,L.Draggable.MOVE[i],this._onMove).off(document,L.Draggable.END[i],this._onUp)}
...
var map=this._map,bounds=map.getPixelBounds(),zoom=map.getZoom(),tileSize=this._getTileSize();if(isNaN(map.getZoom())){zoom=this.options.minZoom||1;map.setZoom(zoom)}
if(zoom>this.options.maxZoom||zoom<this.options.minZoom){return}
var tileBounds=L.bounds(bounds.min.divideBy(tileSize)._floor(),bounds.max.divideBy(tileSize)._floor());this._addTilesFromCenterOut(tileBounds);if(this.options.unloadInvisibleTiles||this.options.reuseTiles){this._removeOtherTiles(tileBounds)}},_addTilesFromCenterOut:function(bounds){var queue=[],center=bounds.getCenter();var j,i,point;for(j=bounds.min.y;j<=bounds.max.y;j++){for(i=bounds.min.x;i<=bounds.max.x;i++){point=new L.Point(i,j);if(!this._tilePointIsCached()&&this._tileShouldBeLoaded(point)){queue.push(point)}}}
...
this._tileContainer.appendChild(fragment)},_tilePointIsCached:function(tilePoint){if(typeof tilePoint=="undefined")return!1;return tilePoint.x+":"+tilePoint.y in this._tiles},_tileShouldBeLoaded:function(tilePoint){var options=this.options;if(!options.continuousWorld){var limit=this._getWrapTileNum();if(options.noWrap&&(tilePoint.x<0||tilePoint.x>=limit.x)||tilePoint.y<0||tilePoint.y>=limit.y){return!1}}
if(options.bounds){var tileSize=options.tileSize,nwPoint=tilePoint.multiplyBy(tileSize),sePoint=nwPoint.add([tileSize,tileSize]),nw=this._map.unproject(nwPoint),se=this._map.unproject(sePoint);if(!options.continuousWorld&&!options.noWrap){nw=nw.wrap();se=se.wrap()}
...
layer._tileLoaded()}});L.tileLayer=function(url,options){return new L.TileLayer(url,options)};L.TileLayer.Canvas=L.TileLayer.extend({options:{async:!1},initialize:function(options){L.setOptions(this,options)},redraw:function(){if(this._map){this._reset({hard:!0});this._update()}
...
return this},update:function(){if(this._icon){var pos=this._map.latLngToLayerPoint(this._latlng).round();this._setPos(pos)}
...
this._setIconStyles(div,"icon");return div},createShadow:function(){return null}});L.divIcon=function(options){return new L.DivIcon(options)};L.LayerGroup=L.Class.extend({initialize:function(layers){this._layers={};var i,len;if(layers){for(i=0,len=layers.length;i<len;i++){this.addLayer(layers[i])}}},addLayer:function(layer){var id=this.getLayerId(layer);this._layers[id]=layer;if(this._map){this._map.addLayer(layer)}
...
return this},setStyle:function(style){return this.invoke("setStyle",style)},bringToFront:function(){return this.invoke("bringToFront")},bringToBack:function(){return this.invoke("bringToBack")},getBounds:function(){var bounds=new L.LatLngBounds;this.eachLayer(function(layer){bounds.extend(layer instanceof L.Marker?layer.getLatLng():layer.getBounds())});return bounds},_propagateEvent:function(e){e=L.extend({layer:e.target,target:this},e);this.fire(e.type,e)}});L.featureGroup=function(layers){return new L.FeatureGroup(layers)};L.Path=L.Class.extend({includes:[L.Mixin.Events],statics:{CLIP_PADDING:function(){var max=L.Browser.mobile?1280:2e3,target=(max/Math.max(window.outerWidth,window.outerHeight)-1)/2;return Math.max(0,Math.min(.5,target))}()},options:{stroke:!0,color:"#0033ff",dashArray:null,lineCap:null,lineJoin:null,weight:5,opacity:.5,fill:!1,fillColor:null,fillOpacity:.2,clickable:!0},initialize:function(options){L.setOptions(this,options)},onAdd:function(map){this._map=map;if(!this._container){this._initElements();this._initEvents()}
...
var ctx=this._ctx,options=this.options;this._drawPath();ctx.save();this._updateStyle();if(options.fill){ctx.globalAlpha=options.fillOpacity;ctx.fill()}
if(options.stroke){ctx.globalAlpha=options.opacity;ctx.stroke()}
ctx.restore()},_initEvents:function(){if(this.options.clickable){this._map.on("mousemove",this._onMouseMove,this);this._map.on("click",this._onClick,this)}},_onClick:function(e){if(this._containsPoint(e.layerPoint)){this.fire("click",e)}},_onMouseMove:function(e){if(!this._map||this._map._animatingZoom){return}
...
if(L.Browser.svg){return "M"+p.x+","+(p.y-r)+"A"+r+","+r+",0,1,1,"+(p.x-.1)+","+(p.y-r)+" z"}else{p._round();r=Math.round(r);return "AL "+p.x+","+p.y+" "+r+","+r+" 0,"+65535*360}},getRadius:function(){return this._mRadius},_getLatRadius:function(){return this._mRadius/40075017*360},_getLngRadius:function(){return this._getLatRadius()/Math.cos(L.LatLng.DEG_TO_RAD*this._latlng.lat)},_checkIfEmpty:function(){if(!this._map){return!1}
...
this._holdTimeout=setTimeout(L.bind(function(){if(this._isTapValid()){this._fireClick=!1;this._onUp();this._simulateEvent("contextmenu",first)}},this),1e3);L.DomEvent.on(document,"touchmove",this._onMove,this).on(document,"touchend",this._onUp,this)},_onUp:function(e){clearTimeout(this._holdTimeout);L.DomEvent.off(document,"touchmove",this._onMove,this).off(document,"touchend",this._onUp,this);if(this._fireClick&&e&&e.changedTouches){var first=e.changedTouches[0],el=first.target;if(el&&el.tagName&&el.tagName.toLowerCase()==="a"){L.DomUtil.removeClass(el,"leaflet-active")}
...
this._container.innerHTML=prefixAndAttribs.join(" | ")},_onLayerAdd:function(e){if(e.layer.getAttribution){this.addAttribution(e.layer.getAttribution())}},_onLayerRemove:function(e){if(e.layer.getAttribution){this.removeAttribution(e.layer.getAttribution())}}});L.Map.mergeOptions({attributionControl:!0});L.Map.addInitHook(function(){if(this.options.attributionControl){this.attributionControl=(new L.Control.Attribution).addTo(this)}});L.control.attribution=function(options){return new L.Control.Attribution(options)};L.PosAnimation=L.Class.extend({includes:L.Mixin.Events,run:function(el,newPos,duration,easeLinearity){this.stop();this._el=el;this._inProgress=!0;this._newPos=newPos;this.fire("start");el.style[L.DomUtil.TRANSITION]="all "+(duration||.25)+"s cubic-bezier(0,0,"+(easeLinearity||.5)+",1)";L.DomEvent.on(el,L.DomUtil.TRANSITION_END,this._onTransitionEnd,this);L.DomUtil.setPosition(el,newPos);L.Util.falseFn(el.offsetWidth);this._stepTimer=setInterval(L.bind(this._onStep,this),50)},stop:function(){if(!this._inProgress){return}
...
if(options.animate!==!1){L.DomUtil.addClass(this._mapPane,"leaflet-pan-anim");var newPos=this._getMapPanePos().subtract(offset);this._panAnim.run(this._mapPane,newPos,options.duration||.25,options.easeLinearity)}else{this._rawPanBy(offset);this.fire("move").fire("moveend")}
...
this.fire("movestart").fire("zoomstart");this._animateZoom(center,zoom,origin,scale,null,!0);return!0},_animateZoom:function(center,zoom,origin,scale,delta,backwards){this._animatingZoom=!0;L.DomUtil.addClass(this._mapPane,"leaflet-zoom-anim");this._animateToCenter=center;this._animateToZoom=zoom;if(L.Draggable){L.Draggable._disabled=!0}
this.fire("zoomanim",{center:center,zoom:zoom,origin:origin,scale:scale,delta:delta,backwards:backwards})},_onZoomTransitionEnd:function(){this._animatingZoom=!1;L.DomUtil.removeClass(this._mapPane,"leaflet-zoom-anim");this._resetView(this._animateToCenter,this._animateToZoom,!0,!0);if(L.Draggable){L.Draggable._disabled=!1}}});L.TileLayer.include({_animateZoom:function(e){if(!this._animating){this._animating=!0;this._prepareBgBuffer()}
var bg=this._bgBuffer,transform=L.DomUtil.TRANSFORM,initialTransform=e.delta?L.DomUtil.getTranslateString(e.delta):bg.style[transform],scaleStr=L.DomUtil.getScaleString(e.scale,e.origin);bg.style[transform]=e.backwards?scaleStr+" "+initialTransform:initialTransform+" "+scaleStr},_endZoomAnim:function(){var front=this._tileContainer,bg=this._bgBuffer;front.style.visibility="";front.parentNode.appendChild(front);L.Util.falseFn(bg.offsetWidth);this._animating=!1},_clearBgBuffer:function(){var map=this._map;if(map&&!map._animatingZoom&&!map.touchZoom._zooming){this._bgBuffer.innerHTML="";this._bgBuffer.style[L.DomUtil.TRANSFORM]=""}},_prepareBgBuffer:function(){var front=this._tileContainer,bg=this._bgBuffer;var bgLoaded=this._getLoadedTilesPercentage(bg),frontLoaded=this._getLoadedTilesPercentage(front);if(bg&&bgLoaded>.5&&frontLoaded<.5){front.style.visibility="hidden";this._stopLoadingImages(front);return}
...
if(this.options.autoToggleDisplay){if(this._mainMap.getBounds().contains(this._miniMap.getBounds())){return!0}
return!1}
return this._minimized}});L.Map.mergeOptions({miniMapControl:!1});L.Map.addInitHook(function(){if(this.options.miniMapControl){this.miniMapControl=(new L.Control.MiniMap).addTo(this)}});L.control.minimap=function(options){return new L.Control.MiniMap(options)};(function(exports){var SUBDOMAINS="a b c d".split(" "),MAKE_PROVIDER=function(layer,type,minZoom,maxZoom){return{url:["//stamen-tiles-{S}.a.ssl.fastly.net/",layer,"/{Z}/{X}/{Y}.",type].join(""),type:type,subdomains:SUBDOMAINS.slice(),minZoom:minZoom,maxZoom:maxZoom,attribution:["<a href='http://leafletjs.com' title='A JS library for interactive maps'>Leaflet</a> | ",'Map tiles by <a href="http://stamen.com">Stamen Design</a>, ','under <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a>. ','Data by <a href="http://openstreetmap.org">OpenStreetMap</a>, ','under <a href="http://creativecommons.org/licenses/by-sa/3.0">CC BY SA</a>.'].join("")}},PROVIDERS={toner:MAKE_PROVIDER("toner","png",0,20),terrain:MAKE_PROVIDER("terrain","jpg",4,18),watercolor:MAKE_PROVIDER("watercolor","jpg",1,16),"trees-cabs-crime":{url:"http://{S}.tiles.mapbox.com/v3/stamen.trees-cabs-crime/{Z}/{X}/{Y}.png",type:"png",subdomains:"a b c d".split(" "),minZoom:11,maxZoom:18,extent:[{lat:37.853,lon:-122.577},{lat:37.684,lon:-122.313}],attribution:['Design by Shawn Allen at <a href="http://stamen.com">Stamen</a>.','Data courtesy of <a href="http://fuf.net">FuF</a>,','<a href="http://www.yellowcabsf.com">Yellow Cab</a>','&amp; <a href="http://sf-police.org">SFPD</a>.'].join(" ")}};setupFlavors("toner",["hybrid","labels","lines","background","lite"]);setupFlavors("toner",["2010"]);setupFlavors("toner",["2011","2011-lines","2011-labels","2011-lite"]);setupFlavors("terrain",["background"]);setupFlavors("terrain",["labels","lines"],"png");exports.stamen=exports.stamen||{};exports.stamen.tile=exports.stamen.tile||{};exports.stamen.tile.providers=PROVIDERS;exports.stamen.tile.getProvider=getProvider;function setupFlavors(base,flavors,type){var provider=getProvider(base);for(var i=0;i<flavors.length;i++){var flavor=[base,flavors[i]].join("-");PROVIDERS[flavor]=MAKE_PROVIDER(flavor,type||provider.type,provider.minZoom,provider.maxZoom)}}
function getProvider(name){if(name in PROVIDERS){return PROVIDERS[name]}else{throw "No such provider ("+name+")"}}
if(typeof L==="object"){L.StamenTileLayer=L.TileLayer.extend({initialize:function(name,options){var provider=getProvider(name),url=provider.url.replace(/({[A-Z]})/g,function(s){return s.toLowerCase()}),_options={minZoom:provider.minZoom,maxZoom:provider.maxZoom,subdomains:provider.subdomains,scheme:"xyz",attribution:provider.attribution};if(options){VCO.Util.mergeData(_options,options)}
L.TileLayer.prototype.initialize.call(this,url,_options)}})}})(typeof exports==="undefined"?this:exports);VCO.MapMarker=VCO.Class.extend({includes:[VCO.Events],initialize:function(data,options){this._el={container:{},content_container:{},content:{}};this._marker={};this._icon={};this._custom_icon=!1;this._custom_icon_url="";this._custom_image_icon=!1;this.marker_number=0;this.media_icon_class="";this.timer={};this.data={};this.options={duration:1e3,ease:VCO.Ease.easeInSpline,width:600,height:600,map_popup:!1};this.animator=null;VCO.Util.mergeData(this.options,options);VCO.Util.mergeData(this.data,data);this._initLayout()},show:function(){},hide:function(){},addTo:function(m){this._addTo(m)},removeFrom:function(m){this._removeFrom(m)},updateDisplay:function(w,h,a){this._updateDisplay(w,h,a)},createMarker:function(d,o){this._createMarker(d,o)},createPopup:function(d,o){this._createPopup(d,o)},active:function(a){this._active(a)},location:function(){return this._location()},_createMarker:function(d,o){},_addTo:function(m){},_removeFrom:function(m){},_createPopup:function(d,o){},_active:function(a){},_location:function(){return{lat:0,lng:0}},_onMarkerClick:function(e){this.fire("markerclick",{marker_number:this.marker_number})},_initLayout:function(){this._createMarker(this.data,this.options)},_updateDisplay:function(width,height,animate){}});VCO.Map=VCO.Class.extend({includes:[VCO.Events,VCO.DomMixins],_el:{},initialize:function(elem,data,options){this._el={container:{},map:{},map_mask:{}};if(typeof elem==="object"){this._el.container=elem}else{this._el.container=VCO.Dom.get(elem)}
this._loaded={data:!1,map:!1};this._map=null;this._mini_map=null;this._markers=[];this.zoom_min_max={min:null,max:null};this._line=null;this._line_active=null;this.current_marker=0;this.bounds_array=null;this._tile_layer=null;this._tile_layer_mini=null;this._image_layer=null;this.data={uniqueid:"",slides:[{test:"yes"},{test:"yes"},{test:"yes"}]};this.options={map_type:"stamen:toner-lite",map_as_image:!1,map_mini:!1,map_background_color:"#d9d9d9",map_subdomains:"",map_access_token:"",zoomify:{path:"",width:"",height:"",tolerance:.8,attribution:""},skinny_size:650,less_bounce:!0,path_gfx:"gfx",start_at_slide:0,map_popup:!1,zoom_distance:100,calculate_zoom:!0,line_follows_path:!0,line_color:"#333",line_color_inactive:"#000",line_weight:5,line_opacity:.2,line_dash:"5,5",line_join:"miter",show_lines:!0,show_history_line:!0,map_center_offset:null};this.animator=null;this.timer=null;this.touch_scale=1;this.scroll={start_time:null};VCO.Util.mergeData(this.options,options);VCO.Util.mergeData(this.data,data);this._initLayout();this._initEvents();this._createMap();this._initData()},updateDisplay:function(w,h,animate,d,offset){this._updateDisplay(w,h,animate,d,offset)},goTo:function(n,change){if(n<this._markers.length&&n>=0){var zoom=0,previous_marker=this.current_marker;this.current_marker=n;var marker=this._markers[this.current_marker];if(this.animator){this.animator.stop()}
this._resetMarkersActive();if(marker.data.type&&marker.data.type=="overview"){this._markerOverview();if(!change){this._onMarkerChange()}}else{marker.active(!0);if(change){if(marker.data.location){this._viewTo(marker.data.location)}else{}}else{if(marker.data.location&&marker.data.location.lat){zoom=this._calculateZoomChange(this._getMapCenter(!0),marker.location());this._viewTo(marker.data.location,{calculate_zoom:this.options.calculate_zoom,zoom:zoom});if(this.options.line_follows_path){if(this.options.show_history_line&&marker.data.real_marker&&this._markers[previous_marker].data.real_marker){var lines_array=[],line_num=previous_marker,point;if(line_num<this.current_marker){while(line_num<this.current_marker){if(this._markers[line_num].data.location&&this._markers[line_num].data.location.lat){point={lat:this._markers[line_num].data.location.lat,lon:this._markers[line_num].data.location.lon};lines_array.push(point)}
line_num++}}else if(line_num>this.current_marker){while(line_num>this.current_marker){if(this._markers[line_num].data.location&&this._markers[line_num].data.location.lat){point={lat:this._markers[line_num].data.location.lat,lon:this._markers[line_num].data.location.lon};lines_array.push(point)}
line_num--}}
lines_array.push({lat:marker.data.location.lat,lon:marker.data.location.lon});this._replaceLines(this._line_active,lines_array)}}else{if(this.options.show_history_line&&marker.data.real_marker&&this._markers[previous_marker].data.real_marker){this._replaceLines(this._line_active,[{lat:marker.data.location.lat,lon:marker.data.location.lon},{lat:this._markers[previous_marker].data.location.lat,lon:this._markers[previous_marker].data.location.lon}])}}}else{this._markerOverview();if(!change){this._onMarkerChange()}}
this._onMarkerChange()}}}},panTo:function(loc,animate){this._panTo(loc,animate)},zoomTo:function(z,animate){this._zoomTo(z,animate)},viewTo:function(loc,opts){this._viewTo(loc,opts)},getBoundsZoom:function(m1,m2,inside,padding){this.__getBoundsZoom(m1,m2,inside,padding)},markerOverview:function(){this._markerOverview()},calculateMarkerZooms:function(){this._calculateMarkerZooms()},createMiniMap:function(){this._createMiniMap()},setMapOffset:function(left,top){this.options.map_center_offset.left=left;this.options.map_center_offset.top=top},calculateMinMaxZoom:function(){for(var i=0;i<this._markers.length;i++){if(this._markers[i].data.location&&this._markers[i].data.location.zoom){this.updateMinMaxZoom(this._markers[i].data.location.zoom)}}
trace("MAX ZOOM: "+this.zoom_min_max.max+" MIN ZOOM: "+this.zoom_min_max.min)},updateMinMaxZoom:function(zoom){if(!this.zoom_min_max.max){this.zoom_min_max.max=zoom}
if(!this.zoom_min_max.min){this.zoom_min_max.min=zoom}
if(this.zoom_min_max.max<zoom){this.zoom_min_max.max=zoom}
if(this.zoom_min_max.min>zoom){this.zoom_min_max.min=zoom}},initialMapLocation:function(){if(this._loaded.data&&this._loaded.map){this.goTo(this.options.start_at_slide,!0);this._initialMapLocation()}},show:function(){},hide:function(){},addTo:function(container){container.appendChild(this._el.container);this.onAdd()},removeFrom:function(container){container.removeChild(this._el.container);this.onRemove()},createMarkers:function(array){this._createMarkers(array)},createMarker:function(d){this._createMarker(d)},_destroyMarker:function(marker){this._removeMarker(marker);for(var i=0;i<this._markers.length;i++){if(this._markers[i]==marker){this._markers.splice(i,1)}}
this.fire("markerRemoved",marker)},_createMarkers:function(array){for(var i=0;i<array.length;i++){this._createMarker(array[i]);if(array[i].location&&array[i].location.lat&&this.options.show_lines){this._addToLine(this._line,array[i])}}},_createLines:function(array){},_createMap:function(){},_createMiniMap:function(){},_createMarker:function(d){var marker={};marker.on("markerclick",this._onMarkerClick);this._addMarker(marker);this._markers.push(marker);marker.marker_number=this._markers.length-1;this.fire("markerAdded",marker)},_addMarker:function(marker){},_removeMarker:function(marker){},_resetMarkersActive:function(){for(var i=0;i<this._markers.length;i++){this._markers[i].active(!1)}},_calculateMarkerZooms:function(){},_createLine:function(d){return{data:d}},_addToLine:function(line,d){},_replaceLines:function(line,d){},_addLineToMap:function(line){},_panTo:function(loc,animate){},_zoomTo:function(z,animate){},_viewTo:function(loc,opts){},_updateMapDisplay:function(animate,d){},_refreshMap:function(){},_getMapLocation:function(m){return{x:0,y:0}},_getMapZoom:function(){return 1},_getMapCenter:function(){return{lat:0,lng:0}},_getBoundsZoom:function(m1,m2,inside,padding){},_markerOverview:function(){},_initialMapLocation:function(){},_onMarkerChange:function(e){this.fire("change",{current_marker:this.current_marker})},_onMarkerClick:function(e){if(this.current_marker!=e.marker_number){this.goTo(e.marker_number)}},_onMapLoaded:function(e){this._loaded.map=!0;if(this.options.calculate_zoom){this.calculateMarkerZooms()}
this.calculateMinMaxZoom();if(this.options.map_mini&&!VCO.Browser.touch){this.createMiniMap()}
this.initialMapLocation();this.fire("loaded",this.data)},_onWheel:function(e){var self=this;if(e.ctrlKey){var s=Math.exp(-e.deltaY/100);this.touch_scale*=s;e.preventDefault();e.stopPropagation(e)}
if(!this.scroll.start_time){this.scroll.start_time=+new Date}
var time_left=Math.max(40-(+new Date-this.scroll.start_time),0);clearTimeout(this.scroll.timer);this.scroll.timer=setTimeout(function(){self._scollZoom()},time_left)},_scollZoom:function(e){var self=this,current_zoom=this._getMapZoom();this.scroll.start_time=null;clearTimeout(this.scroll.timer);clearTimeout(this.scroll.timer_done);this.scroll.timer_done=setTimeout(function(){self._scollZoomDone()},1e3);this.zoomTo(Math.round(current_zoom*this.touch_scale))},_scollZoomDone:function(e){this.touch_scale=1},_calculateZoomChange:function(origin,destination,correct_for_center){return this._getBoundsZoom(origin,destination,correct_for_center)},_updateDisplay:function(w,h,animate,d){this._updateMapDisplay(animate,d)},_initLayout:function(){this._el.map_mask=VCO.Dom.create("div","vco-map-mask",this._el.container);if(this.options.map_as_image){this._el.map=VCO.Dom.create("div","vco-map-display vco-mapimage-display",this._el.map_mask)}else{this._el.map=VCO.Dom.create("div","vco-map-display",this._el.map_mask)}},_initData:function(){if(this.data.slides){this._createMarkers(this.data.slides);this._resetMarkersActive();this._markers[this.current_marker].active(!0);this._loaded.data=!0;this._initialMapLocation()}},_initEvents:function(){var self=this;this._el.map.addEventListener("wheel",function(e){self._onWheel(e)})}});VCO.MapMarker.Leaflet=VCO.MapMarker.extend({_createMarker:function(d,o){var icon={};if(d.location&&d.location.lat&&d.location.lon){this.data.real_marker=!0;var use_custom_marker=o.use_custom_markers||d.location.use_custom_marker;if(use_custom_marker&&d.location.icon){this._custom_icon={url:d.location.icon,size:d.location.iconSize||[48,48],anchor:this._customIconAnchor(d.location.iconSize)};this._icon=this._createIcon()}else if(use_custom_marker&&d.location.image){this._custom_image_icon=d.location.image;this._icon=this._createImage()}else{this._icon=this._createDefaultIcon(!1)}
...
if(this.data.real_marker){if(a){this._marker.setZIndexOffset(100)}else{clearTimeout(this.timer);this._marker.setZIndexOffset(0)}
if(this._custom_icon){this._icon=this._createIcon()}else if(this._custom_image_icon){this._icon=this._createImage(a)}else{this._icon=this._createDefaultIcon(a)}
this._setIcon()}},_createIcon:function(){return new L.icon({iconUrl:this._custom_icon.url,iconSize:this._custom_icon.size,iconAnchor:this._custom_icon.anchor})},_createImage:function(active){var className=active?"vco-mapmarker-image-icon-active":"vco-mapmarker-image-icon";return new L.icon({iconUrl:url,iconSize:[48],iconAnchor:[24,48],shadowSize:[68,95],shadowAnchor:[22,94],className:className})},_createDefaultIcon:function(active){var className=active?"vco-mapmarker-active":"vco-mapmarker";return L.divIcon({className:className+" "+this.media_icon_class,iconAnchor:[10,10]})},_customIconAnchor:function(size){if(size){return[size[0]*.5,size[1]]}else{return[24,48]}},_openPopup:function(){this._marker.openPopup()},_setIcon:function(){this._marker.setIcon(this._icon)},_location:function(){if(this.data.real_marker){return this._marker.getLatLng()}else{return{}}}});VCO.Map.Leaflet=VCO.Map.extend({includes:[VCO.Events],_createMap:function(){this._map=new L.map(this._el.map,{scrollWheelZoom:!1,zoomControl:!this.options.map_mini});this._map.on("load",this._onMapLoaded,this);this._map.on("moveend",this._onMapMoveEnd,this);this._map.attributionControl.setPrefix("<a href='#' class='vco-knightlab-brand'><span>&#x25a0;</span> Made with StoryMapJS</a>");var map_type_arr=this.options.map_type.split(":");this._tile_layer=this._createTileLayer(this.options.map_type);this._tile_layer.on("load",this._onTilesLoaded,this);this._map.addLayer(this._tile_layer);if(this._image_layer){this._map.addLayer(this._image_layer)}
this._line=this._createLine(this._line);this._line.setStyle({color:this.options.line_color_inactive});this._addLineToMap(this._line);this._line_active=this._createLine(this._line_active);this._line_active.setStyle({opacity:1});this._addLineToMap(this._line_active);if(this.options.map_as_image){this._line_active.setStyle({opacity:0});this._line.setStyle({opacity:0})}},_createMiniMap:function(){if(this.options.map_as_image){this.zoom_min_max.min=0}
if(!this.bounds_array){this.bounds_array=this._getAllMarkersBounds(this._markers)}
this._tile_layer_mini=this._createTileLayer(this.options.map_type);this._mini_map=new L.Control.MiniMap(this._tile_layer_mini,{width:150,height:100,position:"topleft",bounds_array:this.bounds_array,zoomLevelFixed:this.zoom_min_max.min,zoomAnimation:!0,aimingRectOptions:{fillColor:"#FFFFFF",color:"#FFFFFF",opacity:.4,weight:1,stroke:!0}}).addTo(this._map);this._mini_map.getContainer().style.backgroundColor=this.options.map_background_color},_createBackgroundMap:function(tiles){trace("CREATE BACKGROUND MAP");if(!this._image_layer){this._image_layer=new L.layerGroup;this._map.addLayer(this._image_layer)}else{this._image_layer.clearLayers()}
if(tiles){for(x in tiles){var target_tile=tiles[x],image={},tile={x:0,y:0,url:target_tile.src,height:parseInt(target_tile.style.height.split("px")[0]),width:parseInt(target_tile.style.width.split("px")[0]),pos:{start:0,end:0}};if(target_tile.style.left||target_tile.style.top){if(target_tile.style.left){tile.x=parseInt(target_tile.style.left.split("px")[0])}
if(target_tile.style.top){tile.y=parseInt(target_tile.style.top.split("px")[0])}}else if(target_tile.style["-webkit-transform"]||target_tile.style.transform||target_tile.style["-ms-transform"]){var t_array;if(target_tile.style["-webkit-transform"]){t_array=target_tile.style["-webkit-transform"].split("3d(")[1].split(", 0)")[0].split(", ")}else if(target_tile.style.transform){t_array=target_tile.style.transform.split("3d(")[1].split(", 0)")[0].split(", ")}else if(target_tile.style["-ms-transform"]){t_array=target_tile.style["-ms-transform"].split("3d(")[1].split(", 0)")[0].split(", ")}
tile.x=parseInt(t_array[0].split("px")[0]);tile.y=parseInt(t_array[1].split("px")[0])}
if(tile.url.match("toner")){tile.url=tile.url.replace("/toner-hybrid/","/toner-lines/");tile.url=tile.url.replace("/toner/","/toner-background/")}
tile.pos.start=this._map.containerPointToLatLng([tile.x,tile.y]);tile.pos.end=this._map.containerPointToLatLng([tile.x+tile.width,tile.y+tile.height]);image=new L.imageOverlay(tile.url,[tile.pos.start,tile.pos.end]);this._image_layer.addLayer(image)}}},_createTileLayer:function(map_type,options){var _tilelayer=null,_map_type_arr=map_type.split(":"),_options={},_attribution_knightlab="<a href='#' class='vco-knightlab-brand' title='A JS library for interactive maps'>Leaflet</a> | ";if(options){_options=options}
switch(_map_type_arr[0]){case "mapbox":if(mapbox_name=_map_type_arr[1]){_options.subdomains="abcd";_options.attribution=_attribution_knightlab+"<div class='mapbox-maplogo'></div><a href='https://www.mapbox.com/about/maps/' target='_blank'>Â© Mapbox Â© OpenStreetMap</a>";_tilelayer=new L.TileLayer("https://api.tiles.mapbox.com/v4/"+mapbox_name+"/{z}/{x}/{y}.png?access_token="+this.options.map_access_token,_options);break}else{trace("Mapbox configured but no map name provided.")}
case "stamen":_tilelayer=new L.StamenTileLayer(_map_type_arr[1]||"toner-lite",_options);this._map.getContainer().style.backgroundColor="#FFFFFF";break;case "zoomify":_options.width=this.options.zoomify.width;_options.height=this.options.zoomify.height;_options.tolerance=this.options.zoomify.tolerance||.9;_options.attribution=_attribution_knightlab+this.options.zoomify.attribution;_tilelayer=new L.tileLayer.zoomify(this.options.zoomify.path,_options);break;case "osm":_options.subdomains="ab";_options.attribution=_attribution_knightlab+"Â© <a target='_blank' href='http://www.openstreetmap.org'>OpenStreetMap</a> and contributors, under an <a target='_blank' href='http://www.openstreetmap.org/copyright'>open license</a>";_tilelayer=new L.TileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",_options);break;case "http":case "https":_options.subdomains=this.options.map_subdomains;_options.attribution=_attribution_knightlab+this.options.attribution;_tilelayer=new L.TileLayer(this.options.map_type,_options);break;default:_tilelayer=new L.StamenTileLayer("toner",_options);break}
return _tilelayer},_onMapMoveEnd:function(e){},_onTilesLoaded:function(e){this._createBackgroundMap(e.target._tiles);this._tile_layer.off("load",this._onTilesLoaded,this)},_onMapZoomed:function(e){trace("FIRST ZOOM");this._map.off("zoomend",this._onMapZoomed,this)},_onMapZoom:function(e){},_createMarker:function(d){var marker=new VCO.MapMarker.Leaflet(d,this.options);marker.on("markerclick",this._onMarkerClick,this);this._addMarker(marker);this._markers.push(marker);marker.marker_number=this._markers.length-1;this.fire("markerAdded",marker)},_addMarker:function(marker){marker.addTo(this._map)},_removeMarker:function(marker){},_markerOverview:function(){var _location,_zoom;this._line_active.setStyle({opacity:0});if(this.options.map_type=="zoomify"&&this.options.map_as_image){var _center_zoom=this._tile_layer.getCenterZoom(this._map);_location=_center_zoom.center;if(this.options.map_center_offset&&this.options.map_center_offset.left!=0||this.options.map_center_offset.top!=0){_center_zoom.zoom=_center_zoom.zoom-1;_location=this._getMapCenterOffset(_location,_center_zoom.zoom)}
this._map.setView(_location,_center_zoom.zoom,{pan:{animate:!0,duration:this.options.duration/1e3,easeLinearity:.1},zoom:{animate:!0,duration:this.options.duration/1e3,easeLinearity:.1}})}else{this.bounds_array=this._getAllMarkersBounds(this._markers);if(this.options.map_center_offset&&this.options.map_center_offset.left!=0||this.options.map_center_offset.top!=0){var the_bounds=new L.latLngBounds(this.bounds_array);_location=the_bounds.getCenter();_zoom=this._map.getBoundsZoom(the_bounds);_location=this._getMapCenterOffset(_location,_zoom-1);this._map.setView(_location,_zoom-1,{pan:{animate:!0,duration:this.options.duration/1e3,easeLinearity:.1},zoom:{animate:!0,duration:this.options.duration/1e3,easeLinearity:.1}})}else{this._map.fitBounds(this.bounds_array,{padding:[15,15]})}}
if(this._mini_map){this._mini_map.minimize()}},_getAllMarkersBounds:function(markers_array){var bounds_array=[];for(var i=0;i<markers_array.length;i++){if(markers_array[i].data.real_marker){bounds_array.push([markers_array[i].data.location.lat,markers_array[i].data.location.lon])}}
return bounds_array},_calculateMarkerZooms:function(){for(var i=0;i<this._markers.length;i++){if(this._markers[i].data.location){var marker=this._markers[i],prev_marker,next_marker,marker_location,prev_marker_zoom,next_marker_zoom,calculated_zoom;if(marker.data.type&&marker.data.type=="overview"){marker_location=this._getMapCenter(!0)}else{marker_location=marker.location()}
if(i>0){prev_marker=this._markers[i-1].location()}else{prev_marker=this._getMapCenter(!0)}
prev_marker_zoom=this._calculateZoomChange(prev_marker,marker_location);if(i<this._markers.length-1){next_marker=this._markers[i+1].location()}else{next_marker=this._getMapCenter(!0)}
next_marker_zoom=this._calculateZoomChange(next_marker,marker_location);if(prev_marker_zoom&&prev_marker_zoom<next_marker_zoom){calculated_zoom=prev_marker_zoom}else if(next_marker_zoom){calculated_zoom=next_marker_zoom}else{calculated_zoom=prev_marker_zoom}
if(this.options.map_center_offset&&this.options.map_center_offset.left!=0||this.options.map_center_offset.top!=0){calculated_zoom=calculated_zoom-1}
marker.data.location.zoom=calculated_zoom}}},_createLine:function(d){return new L.Polyline([],{clickable:!1,color:this.options.line_color,weight:this.options.line_weight,opacity:this.options.line_opacity,dashArray:this.options.line_dash,lineJoin:this.options.line_join,className:"vco-map-line"})},_addLineToMap:function(line){this._map.addLayer(line)},_addToLine:function(line,d){line.addLatLng({lon:d.location.lon,lat:d.location.lat})},_replaceLines:function(line,array){line.setLatLngs(array)},_panTo:function(loc,animate){this._map.panTo({lat:loc.lat,lon:loc.lon},{animate:!0,duration:this.options.duration/1e3,easeLinearity:.1})},_zoomTo:function(z,animate){this._map.setZoom(z)},_viewTo:function(loc,opts){var _animate=!0,_duration=this.options.duration/1e3,_zoom=this._getMapZoom(),_location={lat:loc.lat,lon:loc.lon};if(!this.options.map_as_image){this._line_active.setStyle({opacity:1})}
if(loc.zoom){_zoom=loc.zoom}
if(opts){if(opts.duration){if(opts.duration==0){_animate=!1}else{_duration=duration}}
if(opts.zoom&&this.options.calculate_zoom){_zoom=opts.zoom}}
if(this.options.map_center_offset){_location=this._getMapCenterOffset(_location,_zoom)}
this._map.setView(_location,_zoom,{pan:{animate:_animate,duration:_duration,easeLinearity:.1},zoom:{animate:_animate,duration:_duration,easeLinearity:.1}});if(this._mini_map&&this.options.width>this.options.skinny_size){if(_zoom-1<=this.zoom_min_max.min){this._mini_map.minimize()}else{this._mini_map.restore()}}},_getMapLocation:function(m){return this._map.latLngToContainerPoint(m)},_getMapZoom:function(){return this._map.getZoom()},_getMapCenter:function(offset){if(offset){}
return this._map.getCenter()},_getMapCenterOffset:function(location,zoom){var target_point,target_latlng;target_point=this._map.project(location,zoom).subtract([this.options.map_center_offset.left,this.options.map_center_offset.top]);target_latlng=this._map.unproject(target_point,zoom);return target_latlng},_getBoundsZoom:function(origin,destination,correct_for_center){var _origin=origin,_padding=[Math.abs(this.options.map_center_offset.left)*3,Math.abs(this.options.map_center_offset.top)*3];if(correct_for_center){var _lat=_origin.lat+(_origin.lat-destination.lat)/2,_lng=_origin.lng+(_origin.lng-destination.lng)/2;_origin=new L.LatLng(_lat,_lng)}
var bounds=new L.LatLngBounds([_origin,destination]);if(this.options.less_bounce){return this._map.getBoundsZoom(bounds,!1,_padding)}else{return this._map.getBoundsZoom(bounds,!0,_padding)}},_getZoomifyZoom:function(){},_initialMapLocation:function(){this._map.on("zoomend",this._onMapZoomed,this)},_updateMapDisplay:function(animate,d){if(animate){var duration=this.options.duration,self=this;if(d){duration=d}
if(this.timer){clearTimeout(this.timer)}
this.timer=setTimeout(function(){self._refreshMap()},duration)}else{if(!this.timer){this._refreshMap()}}
if(this._mini_map&&this._el.container.offsetWidth<this.options.skinny_size){this._mini_map.true_hide=!0}else if(this._mini_map){this._mini_map.true_hide=!1}},_refreshMap:function(){if(this._map){if(this.timer){clearTimeout(this.timer);this.timer=null}
this._map.invalidateSize();if(this._markers[this.current_marker].data.type&&this._markers[this.current_marker].data.type=="overview"){this._markerOverview()}else{this._viewTo(this._markers[this.current_marker].data.location,{zoom:this._getMapZoom()})}}}});L.Map.include({_tryAnimatedPan:function(center,options){var offset=this._getCenterOffset(center)._floor();this.panBy(offset,options);return!0},_tryAnimatedZoom:function(center,zoom,options){if(typeof this._animateZoom=="undefined"){return!1}
if(this._animatingZoom){return!0}
options=options||{};var scale=this.getZoomScale(zoom),offset=this._getCenterOffset(center)._divideBy(1-1/scale),origin=this._getCenterLayerPoint()._add(offset);this.fire("movestart").fire("zoomstart");this._animateZoom(center,zoom,origin,scale,null,!0);return!0},getBoundsZoom:function(bounds,inside,padding){bounds=L.latLngBounds(bounds);var zoom=this.getMinZoom()-(inside?1:0),minZoom=this.getMinZoom(),maxZoom=this.getMaxZoom(),size=this.getSize(),nw=bounds.getNorthWest(),se=bounds.getSouthEast(),zoomNotFound=!0,boundsSize,zoom_array=[],best_zoom={x:0,y:0},smallest_zoom={},final_zoom=0;padding=L.point(padding||[0,0]);size=this.getSize();for(var i=0;i<maxZoom;i++){zoom++;boundsSize=this.project(se,zoom).subtract(this.project(nw,zoom)).add(padding);zoom_array.push({x:Math.abs(size.x-boundsSize.x),y:Math.abs(size.y-boundsSize.y)})}
smallest_zoom=zoom_array[0];for(var j=0;j<zoom_array.length;j++){if(zoom_array[j].y<=smallest_zoom.y){smallest_zoom.y=zoom_array[j].y;best_zoom.y=j}
if(zoom_array[j].x<=smallest_zoom.x){smallest_zoom.x=zoom_array[j].x;best_zoom.x=j}}
final_zoom=Math.round((best_zoom.y+best_zoom.x)/2);return final_zoom}});L.TileLayer.include({getTiles:function(){return this._tiles}});VCO.StoryMap=VCO.Class.extend({includes:VCO.Events,initialize:function(elem,data,options,listeners){for(key in listeners){var callbacks=listeners[key];if(typeof callbacks=="function"){this.on(key,callbacks)}else{for(var idx in callbacks){this.on(key,callbacks[idx])}}}
var self=this;this.version="0.1.16";this.ready=!1;this._el={container:{},storyslider:{},map:{},menubar:{}};if(typeof elem==="object"){this._el.container=elem}else{this._el.container=VCO.Dom.get(elem)}
this._storyslider={};this._map={};this.map={};this._menubar={};this._loaded={storyslider:!1,map:!1};this.data={};this.options={script_path:VCO.StoryMap.SCRIPT_PATH,height:this._el.container.offsetHeight,width:this._el.container.offsetWidth,layout:"landscape",base_class:"",default_bg_color:{r:256,g:256,b:256},map_size_sticky:2.5,map_center_offset:null,less_bounce:!1,start_at_slide:0,call_to_action:!1,call_to_action_text:"",menubar_height:0,skinny_size:650,relative_date:!1,duration:1e3,ease:VCO.Ease.easeInOutQuint,dragging:!0,trackResize:!0,map_type:"stamen:toner-lite",attribution:"",map_mini:!0,map_subdomains:"",map_as_image:!1,map_access_token:"pk.eyJ1IjoibnVrbmlnaHRsYWIiLCJhIjoiczFmd0hPZyJ9.Y_afrZdAjo3u8sz_r8m2Yw",map_background_color:"#d9d9d9",zoomify:{path:"",width:"",height:"",tolerance:.8,attribution:""},map_height:300,storyslider_height:600,slide_padding_lr:45,slide_default_fade:"0%",menubar_default_y:0,path_gfx:"gfx",map_popup:!1,zoom_distance:100,calculate_zoom:!0,line_follows_path:!0,line_color:"#e30613",line_color_inactive:"#2c2525",line_join:"miter",line_weight:3,line_opacity:.8,line_dash:"5,5",show_lines:!0,show_history_line:!0,api_key_flickr:"f2cc870b4d233dd0a5bfe73fd0d64ef0",language:"en"};this.current_slide=this.options.start_at_slide;this.animator_map=null;this.animator_storyslider=null;VCO.Util.mergeData(this.options,options);this._initData(data);return this},_initData:function(data){var self=this;if(typeof data==="string"){VCO.getJSON(data,function(d){if(d&&d.storymap){VCO.Util.mergeData(self.data,d.storymap)}
self._initOptions()})}else if(typeof data==="object"){if(data.storymap){self.data=data.storymap}else{trace("data must have a storymap property")}
self._initOptions()}else{trace("data has unknown type");self._initOptions()}},_initOptions:function(){var self=this;VCO.Util.updateData(this.options,this.data);if(this.options.layout=="landscape"){this.options.map_center_offset={left:-200,top:0}}
if(this.options.map_type=="zoomify"&&this.options.map_as_image){this.options.map_size_sticky=2}
if(this.options.map_as_image){this.options.calculate_zoom=!1}
if(this.options.relative_date){if(typeof moment!=="undefined"){self._loadLanguage()}else{VCO.Load.js(this.options.script_path+"/library/moment.js",function(){self._loadLanguage();trace("LOAD MOMENTJS")})}}else{self._loadLanguage()}
if(VCO.Browser.chrome){VCO.Load.css(VCO.Util.urljoin(this.options.script_path,"assets/fonts/font.emoji.css"),function(){trace("LOADED EMOJI CSS FOR CHROME")})}},_loadLanguage:function(){var self=this;if(this.options.language=="en"){this.options.language=VCO.Language;self._onDataLoaded()}else{VCO.Load.js(VCO.Util.urljoin(this.options.script_path,"/locale/"+this.options.language+".js"),function(){self._onDataLoaded()})}},goTo:function(n){if(n!=this.current_slide){this.current_slide=n;this._storyslider.goTo(this.current_slide);this._map.goTo(this.current_slide)}},updateDisplay:function(){if(this.ready){this._updateDisplay()}},_initLayout:function(){var self=this;this._el.container.className+=" vco-storymap";this.options.base_class=this._el.container.className;this._el.menubar=VCO.Dom.create("div","vco-menubar",this._el.container);this._el.map=VCO.Dom.create("div","vco-map",this._el.container);this._el.storyslider=VCO.Dom.create("div","vco-storyslider",this._el.container);this.options.width=this._el.container.offsetWidth;this.options.height=this._el.container.offsetHeight;this._el.map.style.height="800px";this._el.storyslider.style.top="1px";this._map=new VCO.Map.Leaflet(this._el.map,this.data,this.options);this.map=this._map._map;this._map.on("loaded",this._onMapLoaded,this);this._el.map.style.backgroundColor=this.options.map_background_color;this._menubar=new VCO.MenuBar(this._el.menubar,this._el.container,this.options);this._storyslider=new VCO.StorySlider(this._el.storyslider,this.data,this.options);this._storyslider.on("loaded",this._onStorySliderLoaded,this);this._storyslider.on("title",this._onTitle,this);this._storyslider.init();if(this.options.layout=="portrait"){this.options.map_height=this.options.height/this.options.map_size_sticky;this.options.storyslider_height=this.options.height-this._el.menubar.offsetHeight-this.options.map_height-1;this._menubar.setSticky(0)}else{this.options.menubar_height=this._el.menubar.offsetHeight;this.options.map_height=this.options.height;this.options.storyslider_height=this.options.height-this._el.menubar.offsetHeight-1;this._menubar.setSticky(this.options.menubar_height)}
this._updateDisplay(this.options.map_height,!0,2e3);this._menubar.show(2e3)},_initEvents:function(){this._menubar.on("collapse",this._onMenuBarCollapse,this);this._menubar.on("back_to_start",this._onBackToStart,this);this._menubar.on("overview",this._onOverview,this);this._storyslider.on("change",this._onSlideChange,this);this._storyslider.on("colorchange",this._onColorChange,this);this._map.on("change",this._onMapChange,this)},_updateDisplay:function(map_height,animate,d){var duration=this.options.duration,display_class=this.options.base_class,self=this;if(d){duration=d}
this.options.width=this._el.container.offsetWidth;this.options.height=this._el.container.offsetHeight;if(this.options.width<=this.options.skinny_size){this.options.layout="portrait"}else{this.options.layout="landscape"}
if(map_height){this.options.map_height=map_height}
if(VCO.Browser.touch){this.options.layout=VCO.Browser.orientation();display_class+=" vco-mobile"}
if(this.options.layout=="portrait"){display_class+=" vco-skinny";this._map.setMapOffset(0,0);this.options.map_height=this.options.height/this.options.map_size_sticky;this.options.storyslider_height=this.options.height-this.options.map_height-1;this._menubar.setSticky(0);display_class+=" vco-layout-portrait";if(animate){if(this.animator_map){this.animator_map.stop()}
this.animator_map=VCO.Animate(this._el.map,{height:this.options.map_height+"px",duration:duration,easing:VCO.Ease.easeOutStrong,complete:function(){self._map.updateDisplay(self.options.width,self.options.map_height,animate,d,self.options.menubar_height)}});if(this.animator_storyslider){this.animator_storyslider.stop()}
this.animator_storyslider=VCO.Animate(this._el.storyslider,{height:this.options.storyslider_height+"px",duration:duration,easing:VCO.Ease.easeOutStrong})}else{this._el.map.style.height=Math.ceil(this.options.map_height)+"px";this._el.storyslider.style.height=this.options.storyslider_height+"px"}
this._menubar.updateDisplay(this.options.width,this.options.height,animate);this._map.updateDisplay(this.options.width,this.options.height,!1);this._storyslider.updateDisplay(this.options.width,this.options.storyslider_height,animate,this.options.layout)}else{display_class+=" vco-layout-landscape";this.options.menubar_height=this._el.menubar.offsetHeight;this.options.map_height=this.options.height;this.options.storyslider_height=this.options.height;this._menubar.setSticky(this.options.menubar_height);this._menubar.setSticky(this.options.menubar_height);this._el.map.style.height=this.options.height+"px";this._map.setMapOffset(-(this.options.width/4),0);this._el.storyslider.style.top=0;this._el.storyslider.style.height=this.options.storyslider_height+"px";this._menubar.updateDisplay(this.options.width,this.options.height,animate);this._map.updateDisplay(this.options.width,this.options.height,animate,d);this._storyslider.updateDisplay(this.options.width/2,this.options.storyslider_height,animate,this.options.layout)}
if(this.options.language.direction=="rtl"){display_class+=" vco-rtl"}else if(VCO.Language.direction=="rtl"){display_class+=" vco-rtl"}
this._el.container.className=display_class},_onDataLoaded:function(e){this.fire("dataloaded");this._initLayout();this._initEvents();this.ready=!0},_onTitle:function(e){this.fire("title",e)},_onColorChange:function(e){if(e.color||e.image){this._menubar.setColor(!0)}else{this._menubar.setColor(!1)}},_onSlideChange:function(e){if(this.current_slide!=e.current_slide){this.current_slide=e.current_slide;this._map.goTo(this.current_slide);this.fire("change",{current_slide:this.current_slide},this)}},_onMapChange:function(e){if(this.current_slide!=e.current_marker){this.current_slide=e.current_marker;this._storyslider.goTo(this.current_slide);this.fire("change",{current_slide:this.current_slide},this)}},_onOverview:function(e){this._map.markerOverview()},_onBackToStart:function(e){this.current_slide=0;this._map.goTo(this.current_slide);this._storyslider.goTo(this.current_slide);this.fire("change",{current_slide:this.current_slide},this)},_onMenuBarCollapse:function(e){this._updateDisplay(e.y,!0)},_onMouseClick:function(e){},_fireMouseEvent:function(e){if(!this._loaded){return}
var type=e.type;type=type==="mouseenter"?"mouseover":type==="mouseleave"?"mouseout":type;if(!this.hasEventListeners(type)){return}
if(type==="contextmenu"){VCO.DomEvent.preventDefault(e)}
